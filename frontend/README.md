...
H8-22 的 `AppUpdateCenter` 常驻消费同一个 `AppUpdateGateway`：更新提示可以覆盖工作台任意页面，“设置与诊断”同时提供 stable 通道状态、进度和主动检查入口。可选发布展示“立即安装/稍后提醒/跳过此版本”，强制发布使用不可关闭提示且不渲染可选决策；策略与 action 不一致、未知字段和原生异常均 fail closed 为固定文案。`main.tsx` 只注入 `TauriAppUpdateGateway`，该适配器只调用上述三个产品 Command。`pnpm test:h8-22-app` 在三次隐藏真实 App 启动中从可见页面控件完成暂缓、跳过、同版本压制、新版本恢复、立即安装和强更下次启动交接；desktop-e2e 安装探针仍不进入发布包，也不替代 macOS/Windows 真实发布签名包验收。当前本机无有效 macOS 发布签名身份且无 Windows 实体环境，双平台覆盖与安装失败恢复保持待验收。
