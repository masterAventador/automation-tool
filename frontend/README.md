...
H8-18 已锁定 Rust `tauri-plugin-updater 2.10.1` 作为 macOS/Windows 更新检查与安装原语；H8-20～H8-22 已在 Rust 注册插件并只开放 `get_app_update_state`、`check_app_update_now`、`decide_app_update` 三个脱敏产品 Command。`src-tauri/src/app_updates.rs` 验证官方 dynamic feed 的 `raw_json` 与通用 `update_contract` v1，并向 `features/app-updates/contracts.ts` 对应的状态闭集投影安全版本、策略和 Artifact 元数据；React 不安装 updater JavaScript binding，`main` Capability 也不授予 updater 权限，所以不能取得下载 URL/签名、安装路径或调用插件原生命令。
...
H8-22 的 `AppUpdates` 由正式 App 组合根统一挂载，设置页的“检查更新”和启动/周期状态轮询都复用 `TauriAppUpdateGateway`。可选更新弹窗只提供立即安装、暂不安装和跳过此版本；强更没有决策按钮；下载、安装、失败和当前版本使用同一封闭状态投影。`pnpm test:h8-22-ui-app` 在隐藏 App 内从设置入口和提示入口验证完整 UI；`pnpm test:h8-22-real-update` 生成临时 Minisign 密钥及 0.1.0/0.2.0/0.3.0 真包，从同一隐藏 App 原按钮调用生产 FastAPI feed 和官方安装器。macOS App archive 已完成真实替换；Windows current-user NSIS 由 `.github/workflows/desktop.yml` 的独立双平台任务执行，尚未取得 Windows GREEN 前 H8-22 不视为完成。runner 只在规范化临时目录持有私钥，结束后精确删除专属 target、dist、AppData、安装记录、服务、进程和端口。
