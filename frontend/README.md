...
H8-22 的 `AppUpdateCenter` 常驻消费同一个 `AppUpdateGateway`：更新提示可以覆盖工作台任意页面，“设置与诊断”同时提供 stable 通道状态、进度和主动检查入口。可选发布展示“立即安装/稍后提醒/跳过此版本”，强制发布使用不可关闭提示且不渲染可选决策；策略与 action 不一致、未知字段和原生异常均 fail closed 为固定文案。`main.tsx` 只注入 `TauriAppUpdateGateway`，该适配器只调用上述三个产品 Command。`pnpm test:h8-22-app` 在三次隐藏真实 App 启动中完成页面决策与安装交接；`pnpm test:h8-22-macos-package` 进一步从 ad-hoc 0.1.0 DMG 复制三份隔离旧 App，在未设置安装探针的 production feed/Rust/official updater 路径完成 0.3.0 真实覆盖、0.2.0 强更重启和 0.4.0 安装失败恢复，并核对版本、二进制哈希与 ad-hoc codesign。临时包、AppData、密钥、服务和端口均在 finally 删除；Windows 实体包及 Developer ID/notarization、Authenticode 正式发布证据仍待补。
