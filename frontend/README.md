...
H8-22 的 `AppUpdateCenter` 常驻消费同一个 `AppUpdateGateway`：更新提示可以覆盖工作台任意页面，“设置与诊断”同时提供 stable 通道状态、进度和主动检查入口。可选发布展示“立即安装/稍后提醒/跳过此版本”，强制发布使用不可关闭提示且不渲染可选决策；策略与 action 不一致、未知字段和原生异常均 fail closed 为固定文案。Rust enum 字段固定以 camelCase 投影下载进度，用户主动检查或决策期间暂停后台状态轮询，避免同一个 native coordinator 并发撞入。`main.tsx` 只注入 `TauriAppUpdateGateway`，该适配器只调用上述三个产品 Command。`pnpm test:h8-21-app` 在三次隐藏真实 App 启动中完成页面决策与安装交接（同一次运行同时是 H8-21 的验收）；`pnpm test:h8-22-macos-package` 进一步从 ad-hoc 0.1.0 DMG 复制三份隔离旧 App，在未设置安装探针的 production feed/Rust/official updater 路径完成 0.3.0 真实覆盖、0.2.0 强更重启和 0.4.0 安装失败恢复，并核对版本、二进制哈希与 ad-hoc codesign。Windows 的 `pnpm test:h8-22-windows-package` 已准备唯一 `currentUser` 普通未签名 NSIS/AppData/二进制的隔离矩阵，要求 Authenticode `NotSigned`、安装后 PE 版本/哈希及 HKCU 安装/卸载记录精确命中，并在每轮用专属卸载器还原；该命令只允许 Windows x86_64，未在 Windows 实机运行前不构成实体证据。所有 runner 都在 finally 删除临时包、AppData、密钥、服务和端口；普通包证据不等同于 Developer ID/notarization 或 Authenticode 发布门禁。

P9-03 将 release Executor 包根从可写 AppData 分离到只读 `.app/Contents/Resources/local-executor/package`；`local-executor/state`、稳定身份、Profile 和凭据仍只位于 AppData。debug 构建继续使用既有 AppData package，正式启动则由 Rust 自行取得 `resource_dir()`，WebView 没有路径或包选择入口。`tauri.macos-candidate.conf.json` 只选择 `app/dmg`，不锁死发布 identity，也不增加 Capability、CSP、插件或测试 Feature；`pnpm test:p9-03-macos-package` 只在一次性生成的验收覆盖中强制 ad-hoc，从 P9-01 spec 构建临时签名 Executor 和真实 production App/DMG，核对逐文件 SHA-256、Manifest、Mach-O、App codesign、DMG verify/只读挂载及 E4-15 制品边界，结束删除全部临时资源且不启动 App。Developer ID Application、公证和 Gatekeeper 面向普通用户分发仍需正式证书验收。
...
P9-07 的 `pnpm test:p9-07-windows-clean-install --installer <绝对路径> --evidence <绝对路径>` 是对应的 Windows x86_64 非提权设备入口，同样不进入自动 CI。它要求 installer/App/uninstaller 的 Authenticode、在线链、Code Signing EKU、时间戳和 signer 一致，NSIS 只能写 `%LOCALAPPDATA%` 与 HKCU；启动环境不含 Python/开发变量，主窗口必须为 per-monitor aware 且当前缩放至少 125%。扫码窗口打开时 runner 核对一个 Job-owned Executor、唯一 Chrome/Edge、App 私有 Profile 和零 Python；第二次启动后只 `Stop-Process` 主 App，禁止递归杀进程，Executor/浏览器必须由 Job Object 自动退出，第三次启动再确认任务恢复和零重放。卸载必须删除程序与 HKCU、保留私有 AppData；ACL 复验后本轮数据进回收站，证据文件关闭继承且只授权当前用户。当前 `NotSigned` 普通候选和 macOS 静态结果都不能通过该入口。
...
B5-04 曾把浏览器枚举接入“设置与诊断”：WebView 只接收 `google_chrome` / `microsoft_edge` 和当前选择，保存 Command 只接受枚举，并在 Rust 内重新发现受信安装后才原子写入 `app_data_dir/settings/browser-selection-v1`。EB-10 已按内置 Chromium 基线删除该页面，`pnpm test:browser-settings-tauri` 及其隐藏 App 验收随之在 T57b 退役——被点的卡片不存在了，留着只会一直报红。前端不得再出现这两个 Command，由 `tests/browser-settings-boundary.test.mjs` 守。
