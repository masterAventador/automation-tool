...
describe("video editing workbench", () => {
  ...
  it("creates a project and edits tracks, clips, captions, audio and transitions", async () => {
    ...
    expect(screen.getByLabelText("轨道1片段1素材引用")).toHaveValue(ARTIFACT_A);
    ...
    await user.click(screen.getByRole("button", { name: "添加音频轨道" }));
    expect(screen.getByText("音频轨道 3")).toBeVisible();
    expect(screen.getByLabelText("轨道3片段1素材引用")).toHaveValue(ARTIFACT_A);
  ...
  it("explains why an incomplete timeline cannot be saved", async () => {
    ...
    await user.clear(screen.getByLabelText("轨道1片段1素材引用"));
    await user.click(screen.getByRole("button", { name: "保存时间轴" }));
