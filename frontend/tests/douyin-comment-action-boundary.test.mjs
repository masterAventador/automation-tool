...
test("A7-11 keeps comment execution ordered, single-shot, and Executor-local", async () => {
  const source = await readFile(
    new URL(
      "backend/src/automation_tool/executor/rpa/douyin/comment_action.py",
      repositoryRoot,
    ),
    "utf8",
  );
  ...
  const click = source.indexOf("comment_submit.click(");
  const verify = source.indexOf("self._ledger.verify_side_effect(");

  assert.match(source, /douyin\.comment-action-execution\.v1/u);
  assert.match(source, /DouyinCommentActionReceipt/u);
  ...
  assert.ok(prepared < dispatch);
  assert.ok(dispatch < click);
  assert.ok(click < verify);
  assert.equal(source.match(/comment_input\.fill\(/gu)?.length, 1);
  assert.equal(source.match(/comment_submit\.click\(/gu)?.length, 1);
