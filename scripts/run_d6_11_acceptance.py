#!/usr/bin/env python3
"""Run D6-11 through a hidden Tauri App and the real network/backend path."""
...
TAURI_CONFIG = FRONTEND_ROOT / "src-tauri" / "tauri.task-target-preview-e2e.conf.json"
CONTROL_PLANE_PORT = 8765
APP_IDENTIFIER = "com.aventador.automationtool.d611acceptance"
ENVIRONMENT_ID = "d611-acceptance"
TASK_KEY = "task:preview:tauri-acceptance"
...
def require_control_plane_port_available() -> None:
    with socket.socket() as listener:
        try:
            ...
            raise RuntimeError("D6-11 requires an unused Control Plane port") from error
...
def require_hidden_tauri_configuration() -> None:
    ...
    if len(windows) != 1 or windows[0].get("visible") is not False:
        raise RuntimeError("D6-11 Tauri acceptance must run with visible=false")
...
def isolated_environment(database_port: int) -> tuple[dict[str, str], str]:
    ...
    database_url = (
        "postgresql+asyncpg://automation_tool_d611:"
        f"{password}@127.0.0.1:{database_port}/automation_tool_d611"
    )
    token, public_key = signed_bootstrap()
    environment.update(
        {
            "AUTOMATION_TOOL_DEV_DB_USER": "unused_d611_dev",
            "AUTOMATION_TOOL_DEV_DB_PASSWORD": secrets.token_hex(24),
            "AUTOMATION_TOOL_DEV_DB_NAME": "unused_d611_dev",
            "AUTOMATION_TOOL_DEV_DB_PORT": str(unused_loopback_port()),
            "AUTOMATION_TOOL_TEST_DB_USER": "automation_tool_d611",
            "AUTOMATION_TOOL_TEST_DB_PASSWORD": password,
            "AUTOMATION_TOOL_TEST_DB_NAME": "automation_tool_d611",
            ...
            "AUTOMATION_TOOL_D611_BOOTSTRAP_TOKEN": token,
            "AUTOMATION_TOOL_D611_ENVIRONMENT_ID": ENVIRONMENT_ID,
        }
    )
    ...
    try:
        while True:
            if app_process.poll() is not None:
                raise RuntimeError("D6-11 hidden App exited before creating its Task")
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError("D6-11 hidden App did not publish its Task in time")
...
async def verify_database_state(
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("D6-11 Task/Attempt did not converge to the confirmed state")
        if len(targets) != 2 or len(exclusions) != 1:
            raise RuntimeError("D6-11 persisted the wrong target selection")
        if exclusions[0]["target_id"] != targets[1]["id"]:
            raise RuntimeError("D6-11 excluded a target other than the App-selected row")
        if (
            ...
            raise RuntimeError("D6-11 confirmation revision is not durable")
        if events != [
            ...
            raise RuntimeError("D6-11 event history is not contiguous")
    finally:
...
def main() -> None:
    ...
    if private_app_data.exists():
        raise RuntimeError("Refusing to reuse an existing D6-11 App data directory")
    project_name = f"automation-tool-d611-{os.getpid()}"
    ...
    try:
        print("[D6-11] Starting isolated PostgreSQL")
        subprocess.run(
        ...
        print("[D6-11] Applying the production Alembic chain")
        subprocess.run(
        ...
        wait_for_control_plane()
        print("[D6-11] Running the real hidden Tauri App")
        app_process = subprocess.Popen(
            ["pnpm", "test:task-target-preview-tauri"],
            cwd=FRONTEND_ROOT,
            env=environment,
        )
        ...
        try:
            ...
            raise RuntimeError("D6-11 hidden App acceptance did not finish") from error
        if app_exit != 0:
            raise RuntimeError("D6-11 hidden App acceptance failed")
        app_process = None
        verify_app_private_data(private_app_data)
        asyncio.run(verify_database_state(database_url, installation_id, task_id))
        print("[D6-11] Hidden-App target preview acceptance passed")
    finally:
        ...
        if executor_thread is not None:
            ...
            if executor_thread.is_alive():
                cleanup_error = RuntimeError("D6-11 formal Executor did not stop")
            elif executor_failures:
                cleanup_error = RuntimeError("D6-11 formal Executor failed")
