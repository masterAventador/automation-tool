...
def _write_zip(path: Path, entries: dict[str, bytes]) -> str:
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as archive:
        for name, payload in entries.items():
            ...
            executable = name.endswith("Google Chrome for Testing")
...
class DistributionTests(unittest.TestCase):
    ...
    def _manifest(self) -> Path:
        return build_distribution_manifest(
            staging=self.staging, target_id="macos-arm64", enforce_archive_lock=False
        )
    ...
    def test_manifest_aggregates_all_locked_runtime_versions(self) -> None:
        ...
        self.assertEqual(document["target"], "macos-arm64")
        self.assertTrue(document["entries"])
        self.assertTrue(
            any(component["name"] == "chrome-for-testing" for component in document["sbom"])
        )
    ...
    def test_default_mode_rejects_synthetic_archive_digest(self) -> None:
        with self.assertRaises(DistributionRejected):
            build_distribution_manifest(staging=self.staging, target_id="macos-arm64")
        self._manifest()
        with self.assertRaises(DistributionRejected):
            verify_distribution(staging=self.staging, target_id="macos-arm64")
    ...
    def test_extra_file_is_rejected(self) -> None:
        self._manifest()
        (self.staging / "chrome-mac-arm64/extra.bin").write_bytes(b"x")
        with self.assertRaises(DistributionRejected):
            verify_distribution(
                staging=self.staging, target_id="macos-arm64", enforce_archive_lock=False
            )
    ...
    def test_extra_browser_binary_is_rejected_by_name(self) -> None:
        self._manifest()
        extra = self.staging / "chrome-mac-arm64/chrome-headless-shell"
    ...
    def test_platform_mismatch_is_rejected(self) -> None:
        ...
        with self.assertRaises(DistributionRejected):
            verify_distribution(
                staging=self.staging, target_id="windows-x86_64", enforce_archive_lock=False
            )
