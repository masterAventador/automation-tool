...
def packaged_browser_executable(application: Path, target_id: str) -> Path:
    """从正式包的发行物 Manifest 解析包内 Chromium，并确认它真的在包里。"""
    manifest = (
    ...
    if not resolved.is_relative_to(application.resolve()):
        fail(f"resolved browser lives outside the package: {resolved}")
    locked = load_staging_contract(STAGING_CONTRACT).browser_version
    if document.get("runtime", {}).get("chromium", {}).get("browser_version") != locked:
        fail("packaged browser version does not match the locked contract")
    return resolved
