...
def excluded_upstream_resources(contract: dict[str, object]) -> tuple[str, ...]:
    """Return the upstream asset directories the release must not ship."""
    build = contract.get("build")
    if not isinstance(build, dict):
        reject("构建工具契约缺失")
    names = build.get("excludedUpstreamResources")
    if not isinstance(names, list) or not names:
        reject("排除上游资源契约缺失")
    if any(
...
def resolve_locked_python(
    contract: dict[str, object],
    environment: dict[str, str],
) -> tuple[str, Path]:
    """Resolve the exact interpreter before uv creates the isolated runtime."""
    expected = contract.get("python")
    if not isinstance(expected, dict):
        reject("Python 契约缺失")
    version = expected.get("version")
    if not isinstance(version, str) or not version:
        reject("Python 契约未精确锁定版本")
    resolved = run(
    ...
    if not resolved or not interpreter.is_file():
        reject("无法定位契约锁定的 Python 解释器")
...
def build_candidate(output: Path) -> MaterialVideoWorkerAudit:
    ...
    with temporary_build_directory() as temporary:
        ...
        expected_python_version, interpreter = resolve_locked_python(
            contract,
            environment,
        )
