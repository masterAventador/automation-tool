...
def require_real_candidate() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="im02-acceptance-") as directory:
        ...
        inventory = json.loads(
            (candidate / "_internal/licenses/material-video-worker-dependencies.json").read_text(
                encoding="utf-8"
            )
        )
...
def require_evidence() -> None:
    ...
    if len(rows) != 1 or not rows[0].endswith("| 🔍 待验收 |"):
        raise AssertionError("IM-02 roadmap status is not pending native validation")


def main() -> int:
    require_contract()
    subprocess.run([sys.executable, "scripts/test_material_video_worker.py"], cwd=ROOT, check=True)
    require_real_candidate()
    require_evidence()
    print(f"IM-02 {platform.system()} isolated frozen Worker acceptance passed")
    return 0
