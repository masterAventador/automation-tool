#!/usr/bin/env python3
"""Run T3-12 through a hidden Tauri App, real Uvicorn, and PostgreSQL."""
...
TAURI_CONFIG = FRONTEND_ROOT / "src-tauri" / "tauri.task-event-stream-e2e.conf.json"
CONTROL_PLANE_PORT = 8765
APP_IDENTIFIER = "com.aventador.automationtool.t312acceptance"
ENVIRONMENT_ID = "t312-acceptance"
TASK_KEY = "task:stream:tauri-acceptance"
...
def require_control_plane_port_available() -> None:
    with socket.socket() as listener:
        try:
            ...
            raise RuntimeError("T3-12 requires an unused local Control Plane port") from error
...
def require_hidden_tauri_configuration() -> None:
    ...
    if len(windows) != 1 or windows[0].get("visible") is not False:
        raise RuntimeError("T3-12 Tauri acceptance must run with a hidden window")
...
def isolated_environment(database_port: int) -> tuple[dict[str, str], str]:
    ...
    database_url = (
        "postgresql+asyncpg://automation_tool_t312:"
        f"{database_password}@127.0.0.1:{database_port}/automation_tool_t312"
    )
    bootstrap_token, bootstrap_public_key = signed_bootstrap()
    environment.update(
        {
            "AUTOMATION_TOOL_DEV_DB_USER": "unused_t312_dev",
            "AUTOMATION_TOOL_DEV_DB_PASSWORD": secrets.token_hex(24),
            "AUTOMATION_TOOL_DEV_DB_NAME": "unused_t312_dev",
            "AUTOMATION_TOOL_DEV_DB_PORT": str(unused_loopback_port()),
            "AUTOMATION_TOOL_TEST_DB_USER": "automation_tool_t312",
            "AUTOMATION_TOOL_TEST_DB_PASSWORD": database_password,
            "AUTOMATION_TOOL_TEST_DB_NAME": "automation_tool_t312",
            ...
            "AUTOMATION_TOOL_T312_BOOTSTRAP_TOKEN": bootstrap_token,
            "AUTOMATION_TOOL_T312_ENVIRONMENT_ID": ENVIRONMENT_ID,
        }
    )
    ...
    try:
        while True:
            if app_process.poll() is not None:
                raise RuntimeError("T3-12 hidden App exited before creating its stream Task")
            ...
            if row is not None and credential_path.is_file():
                try:
                    ...
                    raise RuntimeError("T3-12 App credential vault is unreadable") from error
                return InstallationId.parse(row[0]), TaskId.parse(row[1]), credential
            if time.monotonic() >= deadline:
                raise RuntimeError("T3-12 hidden App did not create its stream Task in time")
    ...
    try:
        async with database.session() as session:
            ...
            if current != TaskStatus.DRAFT.value:
                raise RuntimeError("T3-12 App Task fixture is not draft")
        ...
        return await service.enqueue(
            ...
            idempotency_key="task:t312:offer:1",
...
async def verify_database_state(database_url: str, original: TaskCommandRecord) -> None:
    ...
    try:
        ...
        if [row["sequence"] for row in event_rows] != [1, 2, 3, 4, 5]:
            raise RuntimeError("T3-12 durable App event timeline is invalid")
        if [row["progress_percent"] for row in event_rows] != [None, None, 50, None, None]:
            raise RuntimeError("T3-12 structured progress persistence is invalid")
        if sorted(session_capabilities) != [
            ...
            raise RuntimeError("T3-12 did not use the expected App and Executor Sessions")
    finally:
        await database.close()
...
def main() -> None:
    ...
    if private_app_data.exists():
        raise RuntimeError("Refusing to reuse an existing T3-12 App data directory")

    project_name = f"automation-tool-t312-{os.getpid()}"
    ...
    try:
        print("[T3-12] Starting isolated PostgreSQL")
        subprocess.run(
        ...
        print("[T3-12] Applying the production Alembic migration chain")
        subprocess.run(
        ...
        print("[T3-12] Starting the real Uvicorn boundary in the background")
        ...
        wait_for_control_plane()
        print("[T3-12] Running the real Tauri App with visible=false")
        app_process = subprocess.Popen(
            ["pnpm", "test:task-event-stream-tauri"],
            cwd=FRONTEND_ROOT,
            env=environment,
        )
        ...
        try:
            ...
            raise RuntimeError("T3-12 hidden App acceptance did not finish") from error
        if app_exit != 0:
            raise RuntimeError("T3-12 hidden App acceptance failed")
        app_process = None
        verify_app_private_data(private_app_data)
        asyncio.run(verify_database_state(database_url, original))
        print("[T3-12] Hidden-App SSE resume and terminal-close acceptance passed")
    finally:
