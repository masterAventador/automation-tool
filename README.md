...
当前处于第一期 MVP 实施阶段。Wave 1～Wave 6 的工程主线、Wave 7 A7-01～A7-15 与 Wave 8 H8-01～H8-16E 已完成；桌面发现入口、Installation 单活、服务端逐目标授权投递、Executor 真实动作编排和启动环境诊断缺口均已关闭，真实账号证据继续独立待补，下一项为 H8-16F MVP 规格差异收口。
...
- H8-18 已选定官方 Rust `tauri-plugin-updater 2.10.1` 作为 macOS/Windows 更新包选择、Minisign 验证、下载和安装原语；自有 `app_updates.rs` 与 React `features/app-updates` 只维护不含业务名、URL、签名和私有路径的版本化发布/状态/决策契约。JavaScript updater binding 与 Capability 均未开放，真实检查、缓存和安装仍按 H8-19～H8-22 顺序实施；
