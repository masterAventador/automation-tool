...
- D6-01 已在 Python Local Executor 建立唯一 `douyin.web.v1` 页面 route contract：首页、Session probe 和 general 搜索结果只接受精确官方 HTTPS origin 与 canonical 路径/查询；未知来源、路径、编码或版本组合统一 `unknown` 并保持熔断。一次性私有 Profile 的无头实探没有发现足以宣称可交互版本的稳定 DOM 锚点，因此没有把通用 `#root` 猜成可操作页面，具体搜索 Page Object 由 D6-02 继续实现；
