...
- Local Executor 已有锁定 PyInstaller 构建依赖和确定性 `onedir` spec；正式 `onedir` 现已收集 Python Playwright driver，但不执行 `playwright install`、不捆绑或下载 Chromium/Firefox/WebKit。macOS 冻结 PoC 已沿受信浏览器复验→私有 Profile→单实例锁→生产 Playwright 模块的原始底层链路启动系统 Chrome headed persistent context 并完整关闭；测试探针不进入正式包，也尚未接入 App/业务任务，正式 BrowserRuntime 由 B5-08 承接。macOS/Windows 使用同一只读 GitHub 矩阵且不上传或发布产物；Windows 原生验收仍受 GitHub Billing/Actions spending limit 阻塞；
