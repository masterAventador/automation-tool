...
- A7-03 已建立 `action-authorization.v1` 短期动作授权：Control Plane 用独立 Ed25519 私钥把 Action、Target、Attempt、Task、Installation、Executor、平台动作、固定幂等键和服务端 UTC 授权/截止时间签入 canonical `ataa1` token；Local Executor 只持有固定公钥，先验签再逐字段匹配当前执行意图，并拒绝篡改、错签发者、跨执行器/安装实例/任务/目标/动作、未来超出时钟偏差和到期授权。私钥不进入 App、Executor、SQLite 或系统钥匙串；当前没有新增 HTTP、App IPC 或真实平台副作用，A7-04 才把验签结果接入本机硬限制；
