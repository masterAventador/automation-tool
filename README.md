...
当前处于第一期 MVP 实施阶段。Wave 1～Wave 3 已完成，正在实施 Wave 4 Tauri 与 Local Executor 生命周期。
...
- 正式 `automation-tool-executor` Python 进程入口已建立：只从 stdin 读取一条 16 KiB 内、拒绝重复 key/未知字段的 bootstrap；其中 Rust 每次生成独立 256-bit 本机启动令牌，Python 健康/停止事件只返回域隔离 `atlep1` HMAC 证明，Rust 以常量时间验证，原令牌和短期 `executor.connect` Session 均不进入 argv、环境、日志、stdout 明文或错误。进程使用正式子协议向真实 Control Plane 发送 Hello 和单调 Heartbeat，SIGINT/SIGTERM 后关闭连接；当前收到任务帧会 fail closed，Tauri 监管、持久账本、任务执行和 RPA 分别由 E4-07、E4-11、E4-12 以后接入；
