...
- B5-13 已提供桌面“平台状态”页面：状态查询从真实 App 经固定 Rust operation 调用 Installation-scoped `GET /api/v1/platform-sessions/douyin`，只展示服务端平台/状态/观察时间；打开处理和无参数重新检查则由 Tauri 在本机解析受信浏览器、稳定 current Douyin Profile 与独占 lease，经同一个 signed Executor 的认证 stdin/stdout 调用 `DouyinQrLoginFlow`，健康事实仍通过正式 WebSocket 投影 PostgreSQL。React 不接触凭据、路径、Cookie、页面对象或 headless 参数，动作结果不会伪造服务端快照；生产浏览器保持可见，隐藏真实 App 验收才硬编码无头并确认 App/Executor/Chrome/容器/端口零残留。安全注销按钮在 B5-14 完成完整停机与定向删除时序前保持禁用；
