...
当前处于第一期 MVP 实施阶段。Wave 1～Wave 6 的工程主线、Wave 7 A7-01～A7-15、Wave 8 H8-01～H8-21 与 P9-01 macOS Executor 候选构建已完成；P9-02 的 Windows Executor 隔离构建器、PE/依赖审计及原生验收入口已经就绪，保留 `🔍 待 Windows 实机验收`。P9-03 已把 macOS Executor 作为只读 Tauri Resources 装入真实 production-mode `.app/.dmg`，完成 ad-hoc 签名、Manifest、资源完整性及最小 Capability/CSP 审计，正式 Developer ID/notarization 仍为 `🔍 待验收`；P9-04 的 Windows production-mode NSIS/currentUser 候选配置、只读 Executor Resources 及隔离安装/卸载验收入口也已就绪，保留 `🔍 待 Windows 实机/Authenticode 验收`。P9-05 已用统一 auditor 扫描真实 macOS App/DMG 完整包树，Windows 安装根扫描已接入 P9-04，保留 `🔍 待 Windows 实机验收`。P9-06/P9-07 的显式 macOS/Windows 干净安装验收器均已工程化，等待正式签名包、授权测试账号和完整 P9 本地服务/设备注册条件后在专用设备执行。H8-22 的通用更新 UI、隐藏 App 原入口自动化、macOS ad-hoc 实包升级和 Windows 隔离普通包验收器已经完成，Windows 实机结果及 macOS/Windows 正式发布签名证据同样待补；真实账号证据继续独立待补。
...
- FastAPI OpenAPI 3.1 快照与 `openapi-typescript` DTO 已覆盖产品账号登录/刷新/注销/改密/恢复、Health/Version、Installation 注册/访问、设备凭据生命周期、短期设备 Session 交换、工作台运行状态/结构化指标，以及 Task 创建/列表/详情/事件 SSE/暂停/恢复/取消/紧停，后端/前端分别具备确定性漂移检查；
...
- MVP 规格复审已固化为 `contracts/quality/mvp-spec-review.v1.json`，P9-09 最终验收另由 `contracts/quality/mvp-final-acceptance.v1.json` 逐项锁定产品规划 14 条原文、证据与 blocker。当前严格结论为 7 条自动化确认、4 条待授权真实平台、3 条待正式双平台设备/包，整体保持 `pending_external_acceptance`；没有把隐藏 App、隔离页或工程 runner 冒充 fresh 安装与真实账号事实；
...
- P9-05 的 `audit-release-bundle.mjs` 在 E4-15 主二进制/config/dist/依赖树审计之外扫描完整 release bundle，固定 macOS `Contents/Resources/local-executor/package` 与 Windows `local-executor/package` 布局，拒绝 symlink、特殊文件、资源超限、测试/WDIO 标记、1420 调试端口、开发验证公钥、测试 Session、私钥，以及日志、Profile、Cookie、SQLite、诊断、上传/下载/用户素材。`pnpm --dir frontend test:p9-05-package-audit` 已在真实 macOS App 和 DMG 挂载副本各扫描 304 个文件/204,479,153 bytes 后清理；Windows 安装根同一审计已接入 P9-04，实机运行前保持 `🔍`；
- P9-06 的 `scripts/run_p9_06_acceptance.py` 只接受操作者显式传入的 Developer ID 签名、公证票据和 Gatekeeper 均通过的 DMG，不在设备验收时重建或替换制品。它拒绝 root、非 TTY、已有验收 App 或生产 AppData，在用户级 `Applications` 新装正式 App，以仅含系统目录的 `PATH` 且无 Python/开发变量启动；扫码期间从进程树核对 signed Executor、唯一 Chrome/Edge、App 私有 `browser-profiles` 与零 Python 后代，正常退出后再启动核对 Session/任务恢复和零重复动作。扫码、预览与 browse 结果由授权设备操作者按固定 checkpoint 确认，最终只写不含 PID、路径、Cookie 或页面内容的 `0600` JSON；App 和 AppData 移入废纸篓而非直接删除。该入口没有进入 CI，也没有在当前 ad-hoc/无授权账号环境运行；
- P9-07 的 `scripts/run_p9_07_acceptance.py` 只接受 Authenticode 状态/在线证书链/Code Signing EKU/RFC3161 时间戳全部有效的现成 NSIS，并要求 installer、installed App、uninstaller 为同一 signer。非提权 TTY 从空 `%LOCALAPPDATA%`/`%APPDATA%` 和无 HKCU/HKLM 记录开始，安装后固定要求 currentUser/HKCU-only、P9-05 完整包审计、App 环境零 Python、至少 125% 的 per-monitor DPI aware 主窗口，以及 Job Object 内唯一 Executor 和 App 私有 Chrome/Edge Profile。第二次启动只强停 App 主进程，Executor/浏览器必须因 Job handle 关闭自行消失；第三次启动再确认快照与零重放。正式卸载器必须清除程序/HKCU 且保留私有 AppData，runner 复验 ACL 后把本轮数据移到回收站，并写仅当前用户可读的最小 JSON。该显式可见入口不进 CI，当前 Mac/普通 `NotSigned` 包没有被冒充为 Windows 结果；
