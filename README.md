...
当前处于第一期 MVP 实施阶段。Wave 1～Wave 6 的工程主线、Wave 7 A7-01～A7-15 与 Wave 8 H8-01～H8-21 已完成；H8-22 的通用更新 UI 与隐藏 App 原入口自动化已经完成，macOS/Windows 真实发布签名包升级证据仍为 `🔍 待验收`。桌面 MVP 闭环、恢复诊断、后台更新下载及安全安装协调均已收口，真实账号证据继续独立待补。
...
- H8-22 已在“设置与诊断”加入通用 App 更新卡片，并在任意工作台页面挂载可选/强制更新提示。React 只经三个固定 Tauri Command 读取脱敏状态、主动检查和提交封闭决策；可选更新支持立即安装、稍后提醒和跳过当前版本，强更提示不可关闭且没有暂缓/跳过入口。三轮隐藏真实 App 已从页面点击验证暂缓、同版本压制、新版本覆盖提示、立即安装和强更下次启动交接；当前 macOS arm64 本机没有有效发布代码签名身份，也没有 Windows 实体签名环境，因此双平台真实发布签名包覆盖与失败恢复仍保持 `🔍 待验收`，未用 debug 安装探针冒充完成；
