...
- 正式 `automation-tool-executor` Python 进程入口已建立：只从 stdin 读取一条 16 KiB 内、拒绝重复 key/未知字段的 bootstrap；Rust `ExecutorManager` 在完整签名包复验后从固定入口启动单实例，每次生成独立 256-bit 本机令牌，Python 健康/停止事件只返回域隔离 `atlep1` HMAC 证明，Rust 以常量时间验证，原令牌和短期 `executor.connect` Session 均不进入 argv、环境、日志、stdout 明文或错误。真实 PyInstaller→Manager→Uvicorn 的 start/status/stop 已在 macOS 通过；当前收到任务帧会 fail closed，退出监管、进程树、诊断、持久账本和任务执行分别由 E4-08～E4-12 接入；
