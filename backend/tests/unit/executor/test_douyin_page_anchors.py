...
def test_every_deduplicated_group_stays_css_only(group: tuple[str, ...]) -> None:
    ...
    into ``page_unavailable``. ``unique_visible_in_snapshot`` joins the same
    way and then chains the visible engine with ``>>``, so a group carrying its
    own ``>>`` would also change which element the chain ends on.
