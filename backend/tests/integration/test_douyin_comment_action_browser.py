...
ACTION_ID = ProtocolActionId("523e4567-e89b-42d3-a456-426614174001")
TARGET_ID = ProtocolTargetId("523e4567-e89b-42d3-a456-426614174002")
...
def test_production_comment_action_uses_headless_browser_gate_and_ledger_once(
    ...
    if sys.platform != "darwin" or not MACOS_CHROME.is_file():
        pytest.skip("A7-11 system Chrome fake-page acceptance currently requires macOS Chrome")
    profile = tmp_path / "automation-tool-a7-11-profile"
    profile.mkdir(mode=0o700)
    clock = Clock()
    ledger = ExecutorLedger(
        state_directory=tmp_path / "automation-tool-a7-11-state",
    ...
    intent = DouyinCommentActionIntent(
        ...
        message_template=ActionMessageTemplate(
            source="您好 {{target_display_name}}, 这是隔离验收评论"
        ),
    ...
    with runtime.running(
        ...
        first = DouyinCommentActionExecution(
            window=window,
            action_gate=gate,
            ledger=ledger,
            clock=clock,
        ).run(token=token, intent=intent)
        assert first.state is DouyinCommentActionState.VERIFIED
        assert first.evidence is DouyinCommentActionEvidence.COMMENT_CONFIRMED
        assert page.evaluate("window.__commentDispatches") == 1
        assert page.locator("textarea").input_value() == "您好 测试目标, 这是隔离验收评论"

        replay = DouyinCommentActionExecution(
            window=window,
            action_gate=gate,
            ledger=ledger,
            clock=clock,
