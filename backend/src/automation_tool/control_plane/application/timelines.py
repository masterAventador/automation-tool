...
which is a different thing to report and a different thing to fix. A referenced
material that is absent or belongs to another Installation is the same class of
cross-aggregate mistake. A missing timeline is a question that got an answer.
An unavailable database is worth retrying. A rejected argument is a bug
upstairs. Collapsing them makes every caller guess, and leaves the REST layer
above answering 409, 404 and 503 with the same status.
...
DETAIL line on the integrity failures below quotes the offending key
...
class TimelineMaterialMissing(_TimelinePersistenceFailure):
    """A referenced material is absent or belongs to another Installation."""

    message = "Timeline names a material that is not stored"
