...
B5-13 的隐藏 App 原入口执行 `uv run --project backend python scripts/run_b5_13_acceptance.py`：唯一 `visible=false` Tauri App 实际点击平台状态、打开处理与重新检查，经正式 TypeScript/Rust、signed Executor、后台系统 Chrome、认证 WebSocket 和真实 Uvicorn/Alembic/PostgreSQL 核对最终 projection。runner 使用专属随机端口、Compose project、AppData、SQLite 和 Profile，并在 App 正常退出后审计浏览器、Executor、容器和端口零残留。
