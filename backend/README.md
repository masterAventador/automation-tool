...
H8-06 在现有 `LocalExecutorProcess` 内增加只针对 WebSocket `1012` 服务重启关闭码的有界重连。初次连接失败、非 1012 关闭、协议错误和应用错误仍固定失败；进入恢复态后连接失败按固定预算和延迟重试，停止信号可打断等待，只有恢复连接收到健康心跳才重置预算。进程不退出、不经过 Rust supervisor，也不更换 bootstrap/Session；本机 `recover_outbox()` 复用原 envelope、message ID 与 idempotency key，Control Plane 则继续依靠既有 Command/Event 幂等约束收敛，不新增 API、表或第二套恢复协议。
...
uv run automation-tool-build-macos-executor \
  --backend-root . \
  --output /absolute/new/path/automation-tool-executor
...
PyInstaller 产物固定为 `dist/automation-tool-executor/` 的 `onedir` 目录；macOS 入口为同名可执行文件，Windows 入口为 `automation-tool-executor.exe`。`pyinstaller` 只锁在开发依赖组；macOS framework 包内 symlink 在构建结束后、Manifest 签名前被实体化，只允许同一包根内的普通文件/目录，逃逸、循环、特殊文件或构建中漂移均拒绝。P9-01 的 macOS 候选命令另外使用隔离的 build/cache/dist，拒绝覆盖已有输出，移除 editable install 的 `direct_url.json` 开发路径元数据，并在确认 framework 根别名与版本化 Python 二进制逐字节相同后删除冗余别名；最终目录的全部 Mach-O 统一 ad-hoc 重签并逐个严格复验。该结果只是 P9-03 Developer ID/notarization 前的签名准备，不是正式发布签名。P9-02 的 Windows 候选命令复用同一 spec 和隔离目录策略，复制前后流式拒绝仓库/临时路径、浏览器缓存、symlink/reparse point、特殊文件、错误架构 PE 及缺失的 base library/Playwright driver；正式原生 runner 另验证一次性 Manifest、冻结入口固定失败、只读 UIAutomationClient 系统能力和 E4-09 Job Object 后代树清理。工程门禁已就绪，但真实 PE/UIA/Job Object 结果仍须在 Windows 执行，当前不冒充完成。B5-07 起 Playwright 是 Executor 正式运行依赖，spec 用 `collect_all("playwright")` 收集 Python driver，但仓库和构建流程均不执行 `playwright install`，正式目录不包含 Playwright 浏览器缓存。双平台 CI 从冻结产物验证 bootstrap/连接失败契约及 driver/浏览器分离，另由测试专用冻结探针从生产 `browser_runtime.py` 启动系统 Chrome/Edge headed persistent context；探针不进入正式包、不上传或发布产物。macOS arm64 与 Windows x86_64 实包/系统浏览器原生验收均已通过；Hosted Windows Runner 仍受 GitHub 账户 Billing 限制，但不再是本机产品验收阻塞。
...
D6-14 的 `executor/page_drift_artifact.py` 现在只保留页面漂移固定 Schema 和窄 Policy/Ref，底层复用 H8-09/H8-12 Store，不再维护第二套文件安全或清理实现。它只在 D6-04 明确返回 `page_version_unknown/conflicting_anchors` 时写入 `artifacts/evidence/page-drift/<id>.json`，单文件上限 2 KiB、总数上限 20、保留 30 天；诊断失败不能解除页面熔断，发现结果仍经正式 Executor wire 收敛到 `handoff_required`，Control Plane 投影为 `awaiting_human`。关键词、URL、HTML、页面正文、Cookie、凭据和 Profile 路径没有输入字段；H8-10 的受限截图/Trace 使用独立固定 Policy，自动治理不扩大页面漂移 Schema。
