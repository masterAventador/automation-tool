...
本目录是 Python、Rust 和 TypeScript 共同回放的协议事实源。`valid/` 中 7 个样例必须同时通过 Draft 2020-12 JSON Schema 和正式语义解析器；`invalid/` 中 26 个样例必须由正式解析器统一拒绝。
...
其余 16 个结构层无效样例必须被标准 Draft 2020-12 validator 和正式解析器同时拒绝。Fixture 不含真实 Cookie、Token、私钥、账号、路径或用户数据；隐私拒绝用例中的字段和值都只是显式 `fixture` 测试标记。
