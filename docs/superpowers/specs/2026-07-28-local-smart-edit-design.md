...
### 4.5 模型调用参数：首期一律关闭深度思考

本链路涉及三处百炼调用：素材理解（多模态）、文案分句（文本）、语义匹配（文本）。**三处全部关闭深度思考模式**（qwen 系列的 `enable_thinking` 传 false）。

理由是这三件事都不是推理任务：描述画面里有什么、把一段文案切成句子、判断哪句话配哪段素材，都属于直接判断。开启深度思考会显著拉长响应时间并增加 token 消耗，而对这类任务的输出质量收益不明确——用户等一条草稿的时间是产品体验的一部分，本地渲染只要 5～55 秒，模型环节不该成为新的瓶颈。

该开关必须是**配置项而非硬编码**：后续若发现语义匹配质量不足，可单独为该环节开启并做对比验证，不需要改代码。默认值为关闭。

**用户可以自己开。** 智能剪辑入口的高级选项里提供一个总开关，统一控制三处调用——分三个开关是把内部结构泄漏给用户，他不该需要知道链路里有几处调模型。

开关旁必须显示**开启后预计多花的时间**，且这个数字由实测得出，不允许估计或编造。耗时差大概率随素材条数增长（每条素材一次多模态调用），所以先实测开/关两种模式的端到端耗时：若呈线性则按条数动态展示（形如"约多花 3 秒 × 素材条数"），否则展示实测区间。实测数据与计算方式记入台账 LE-24 的证据文件。

文案不能只写"深度思考"四个字了事——项目规则要求用户可见文案无未解释术语，须说明它做什么以及代价是什么。
