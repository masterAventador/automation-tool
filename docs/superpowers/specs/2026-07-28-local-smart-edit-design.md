...
旁白支持两种来源：TTS 合成（默认）与用户上传录音（转写后对齐到文案句子）。
...
- **机制与字体分两个任务**，避免互相阻塞：fallback 链路本身用现有 Noto Sans SC 加一个拉丁字体即可验证（台账 LE-09），生僻字字体的引入、锁版本与生产装配是独立任务（台账 LE-19）
...
| 层级 | 覆盖 |
| --- | --- |
| pytest 单元 | 领域对象校验、状态机、抽帧策略分档、编排算法、fallback 链选字 |
| pytest 集成 | 真实 PostgreSQL 仓储、真实 ffmpeg 渲染小样本并 ffprobe 断言产物 |
...
其他后续项：变速适配、语音识别区分人声与环境音（"有人说话就不配旁白"）、长视频自动剪成多条短视频。
