...
| BM-05 | AI 一句话到动效视频编排 | 受限 MotionAuthoringAgent 读取锁定 workflow，以封闭工具把 Brief→DESIGN/脚本/分镜→可 seek 的 HTML/动画→lint/check/snapshot→局部修正→提交 RenderJob；逐帧 Chromium/FFmpeg 不调用模型，Agent 无 Shell/任意文件/Profile/密钥权限，未配置模型时自动制作明确不可用 | BM-04,VF-01,VF-05 | 🔍 待验收 |
