...
共 79 个候选小任务，其中 BM-09～BM-10 和 SA-01～SA-07 共 9 项只做后续规划，首期不激活：
...
| EB-18 | Windows 发布打包命令 | `build_release_package.py --platform windows` 从「显式拒绝」变为唯一 Windows 出包入口：部署 Profile、更新端点与发布身份三条通道与 macOS 对齐；`run_eb_16_windows_acceptance.py`、`run_pc_16_windows_package_acceptance.py`、`run_le_23_windows_package_acceptance.py` 三份重复的 `build_installer()` 收敛为调用它，验证路径与出厂路径不得再分叉 | EB-16 | 🧪 RED |
