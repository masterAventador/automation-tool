...
docs/development-roadmap.md 仍是全项目进度和完成证据的总入口；本文件是这个专项的细化执行 Roadmap，除拆分“内置运营浏览器”、Browser Use、“两种视频制作方式”、独立“视频剪辑”、首期双平台发布和后续自愈式自动化外，也逐行维护 87 个任务的当前状态，方便直接追踪专项进度。
...
3. 每次激活、实现、验收或完成任务时，在同一次代码提交中更新本文件对应行的状态，并同步主路线图的全局阶段、证据和提交；两处出现冲突时必须在当前任务内修正，不能长期漂移；
...
一个任务只能有一个当前状态。每次状态变化都要同步上面的进度计数。每完成一个任务，都必须在同一次本地提交中包含代码、测试、失败矩阵、该行状态和主路线图证据；不得先把状态改成完成再补实现，也不得只提交代码而不更新进度。

### 9.1 架构与治理（4 项）

| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| AV-01 | 内置浏览器 ADR 与基线切换 | 写清替代关系、版本策略、全新 Profile、人工接管和首发边界；同步 CLAUDE.md、产品规划、前后端架构、工程结构和主路线图，不静默保留冲突基线 | — | ⬜ 未开始 |
| AV-02 | 第三方源码与许可证治理 | 固定两个 submodule、升级流程、干净工作树检查、许可证、字体/素材/编解码器权利、SBOM 和安全公告处理 | AV-01 | ⬜ 未开始 |
| AV-03 | 威胁模型、术语与用户品牌契约 | 覆盖本地端口、Worker、生成 HTML、素材下载、密钥、路径和名称泄漏；建立用户界面中文词表与禁止词检查 | AV-01 | ⬜ 未开始 |
| AV-04 | 主/专项 Roadmap 状态同步门禁 | 把首批任务和依赖写入 docs/development-roadmap.md；保留旧任务历史，建立同提交状态同步检查，确保两份 Roadmap 指向同一任务且只有一个 RED/实现中任务 | AV-01..AV-03 | ⬜ 未开始 |

### 9.2 内置运营浏览器（17 项）

| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| EB-01 | Playwright 与 Chromium 兼容矩阵 | 锁定 Playwright、完整 Chromium 版本/修订、macOS arm64 和 Windows x86_64；安装包内版本不匹配时 fail closed | AV-04 | ⬜ 未开始 |
| EB-02 | 三方共用 Chromium 前置验证门禁 | 在 macOS/Windows 用 EB-01 同一完整 Chromium 分别启动可见 RPA、Browser Use `executable_path`/随机 CDP 两种模式和独立无头渲染进程；渲染 12 套风格、全部 134 项单帧冒烟、字体/图片/视频/音频/Lottie/Canvas/WebGL/WebGPU、透明画面、横竖屏，并验证三方并发、进程/Profile 隔离、控制权租约和无第二套浏览器下载；通过后 ADR 固定为一套，失败则先调整共同版本重跑并暂停后续任务 | AV-02,EB-01 | ⬜ 未开始 |
...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| BU-01 | Browser Use 版本与 API 契约 | 锁 `browser-use==0.13.6`、Python/依赖、Agent/BrowserSession/Tools/history API 和序列化 fixture；实现前再次核对是否有更新稳定版并单独升级验证 | AV-04 | ⬜ 未开始 |
| BU-02 | 单一 Chromium 双模式适配 | Rust 只下发已验证 `executable_path`；独立模式用临时 Profile，抖音模式用随机 loopback `cdp_url` 接管运营进程；关闭系统浏览器发现、默认安装/下载、Cloud 和 fallback | BU-01,EB-02 | ⬜ 未开始 |
...
| VF-05 | 模型与服务密钥设置 | Rust 管理秘密；React 只见“已配置”；区分“文案模型”和“视频创作模型”，可复用同一服务配置但权限/用途分别说明，并提供连接测试、超时、额度和错误脱敏 | VF-02 | ⬜ 未开始 |
| VF-06 | 左侧“视频制作”入口与页面骨架 | 接入现有 WorkbenchShell，完成新建、脚本/分镜、设置、预览、任务和成片页面；无假数据冒充运行结果 | VF-01 | ⬜ 未开始 |
| VF-07 | 两种制作方式选择器 | 两张中文卡片清楚展示适用/不适用、示例、耗时、资源、网络和隐私；未来方案不展示 | VF-05,VF-06 | ⬜ 未开始 |

### 9.5 智能素材成片（8 项）

| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| IM-01 | 上游 submodule 固定与升级门禁 | vendor/moneyprinterturbo 固定 v1.3.2/b1588e1；递归初始化、离线构建、无上游修改和脏树拒绝 | AV-02 | ⬜ 未开始 |
| IM-02 | 隔离 Python Worker 与依赖打包 | 与 RPA Executor 分开冻结，验证 Python 3.11+、MoviePy、语音/字幕依赖、启动时间、包体和资源许可证 | IM-01,VF-02 | ⬜ 未开始 |
| IM-03 | 安全启动器和本地网关 | 只监听随机 loopback；会话令牌、来源/路径白名单、请求大小、超时、素材文件 containment、CORS 和错误脱敏 | IM-02,VF-03 | ⬜ 未开始 |
| IM-04 | 文案模型服务设置适配 | 把上游 LLM Provider、模型、Base URL 和连接测试映射到 App“文案模型服务”；密钥不进 React、配置和普通日志 | IM-03,VF-05 | ⬜ 未开始 |
| IM-05 | 完整 WebUI 子 WebView 技术验证 | 从“视频制作”正式入口打开受保护 WebUI；功能、表单、生成、进度、取消和结果可用；不暴露原始 localhost | IM-03,IM-04,VF-06 | ⬜ 未开始 |
| IM-06 | 主题统一与上游名称隐藏 | 通过 App 自有主题配置和子 WebView 初始化脚本统一颜色、字体、间距；标题、DOM、可访问性、错误、外链和任务记录无上游品牌，变化时 fail closed | IM-05,AV-03 | ⬜ 未开始 |
| IM-07 | RenderJob 与 Artifact 对账 | WebUI 创建的任务进入 App 状态机；进度、取消、失败、成片摘要、保留和删除只有一个权威来源 | IM-06,VF-03 | ⬜ 未开始 |
| IM-08 | 代表性视频与正式包验收 | 知识讲解、资讯摘要、榜单三类样片；素材权利、字幕、配音、断网、API 失败、磁盘、取消、重启、macOS/Windows 包全部验证 | IM-07,VF-04 | ⬜ 未开始 |

### 9.6 品牌动效成片（16 项）

| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| BM-01 | 上游源码与 12 套风格固定 | 固定 v0.7.68 和 submodule；只登记官网公开 12 套；英文标识到中文名称映射有契约测试 | AV-02 | ⬜ 未开始 |
| BM-02 | 隔离 Node 22 Worker | Tauri sidecar 打包命令行/SDK 适配器，验证版本握手、无全局 Node 前置、取消、退出和错误脱敏 | BM-01,VF-02 | ⬜ 未开始 |
...
| BM-06 | 12 套风格目录与中文选择器 | 全部 12 套有预览、中文名、适用场景、标签和键盘可访问性；少一套、重复或混入未公开模板都失败 | BM-01,VF-07 | ⬜ 未开始 |
...
| BM-11 | 全部 134 项 Catalog 固定与权利审计 | 固定 109 个画面块、25 个局部组件的清单/摘要/中文分类；官网 100 项标记为有官方预览；逐项审计代码、远程依赖、字体、素材、作者、商标、GPU 和再分发权 | BM-01,AV-02 | ⬜ 未开始 |
| BM-12 | 远程依赖离线供应链 | 在不修改 submodule 的前提下生成 `OfflineMotionCatalog`；本地化并锁定 GSAP/Three.js/D3/TopoJSON/Draco/地图数据/开放字体，保存来源、许可证与摘要；134 项静态扫描零远程 URL | BM-11 | ⬜ 未开始 |
| BM-13 | 示例资产去品牌与原创替换 | Image Generation 生成原创头像/壁纸/插画/纹理；地图、3D、音效、字体和 SVG/UI 分别用有明确再分发权的资源或自研中性版本替换；建立资产来源、审核和摘要台账 | BM-11,AV-02 | ⬜ 未开始 |
| BM-14 | 134 项离线目录构建器 | 将上游代码、离线依赖和 App 资产 overlay 合成为只读版本化目录；不回写 submodule，构建可复现，缺项、远程引用、未登记资产或摘要漂移均失败 | BM-12,BM-13 | ⬜ 未开始 |
...
| VE-08 | Provider 一致性与可替换验收 | 用阿里真实沙箱/账号样例和假的第二 Provider 跑同一契约、Timeline 与 UI；证明未来新增腾讯云 Adapter 不改领域层/页面；首期不实现腾讯云 | VE-02..VE-07 | ⬜ 未开始 |

### 9.8 首期内容发布（8 项）

| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| PB-01 | 发布领域与首期 capability 冻结 | 复用 PublishJob/Artifact/审批/ledger；只启用 B站 `official_api` 和抖音 `browser_use`，快手/小红书/视频号明确 `deferred` 且无入口 | AV-04,VF-01 | ⬜ 未开始 |
| PB-02 | B站正式 API 授权与版本契约 | 复核当前开放平台文档/控制台 scope，锁请求/响应 fixture、授权续期、限流、错误和上传规格；凭据缺失时以官方契约、Mock Server 和 fixture 完成开发，不等待密钥；旧接口或非正式接口不得接入 | PB-01 | ⬜ 未开始 |
| PB-03 | B站上传与创建稿件 | 素材校验、分片/断点、封面/标题/简介/分区/标签，幂等准备后只创建一次；保存 resource/稿件 ID 与请求摘要；无真实凭据也必须完成实现、Mock 纵向测试和失败矩阵 | PB-02,VF-03 | ⬜ 未开始 |
| PB-04 | B站状态查询与对账 | 审核中、成功、拒绝、超时、回调/轮询、重启恢复和 `outcome_uncertain`；不得因超时重复投稿；凭据未提供时状态为“代码完成/待凭据验收”，不阻塞 PB-05 及后续任务 | PB-03 | ⬜ 未开始 |
...
| CQ-05 | 文档、SBOM 与主路线图收口 | 更新用户帮助、架构、隐私、许可证、版本锁、故障排查和卸载说明；主路线图记录真实提交和证据后才能完成 | CQ-04 | ⬜ 未开始 |
...
1. 在主路线图检查依赖，并同时把主路线图和本文件中的唯一当前任务置为 RED；
2. 先写能证明目标行为且当前失败的测试，实际运行并记录正确失败；
3. 写最小实现进入 GREEN；
4. 运行任务相关测试、静态检查和适用的正式入口验收；
5. 覆盖任务适用的失败矩阵和资源清理；
6. 同一提交更新代码、测试、本文件任务状态、主路线图状态和证据；
