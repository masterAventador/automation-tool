...
本文件是“内置运营浏览器”、Browser Use、“两种视频制作方式”、独立“视频剪辑”、首期双平台发布和后续自愈式自动化这 87 个专项任务的唯一明细台账。任务状态、提交和验收证据只在本文件维护；`docs/development-roadmap.md` 作为既有项目历史和领域对象来源保持只读，本专项实施期间不向其中复制任务或同步状态，避免与同时在 `main` 开发旧任务的 Session 产生无关冲突。

实施前必须先完成 AV-01 和 AV-04：

1. 用 ADR 和基线文档正式替换“依赖用户安装 Chrome/Edge”的现行方案；
2. 建立只检查本文件的状态与证据门禁，不把 87 个任务复制到 `docs/development-roadmap.md`；
3. 每次激活、实现、验收或完成任务时，在同一次代码提交中更新本文件对应行的状态、证据和提交；
...
一个任务只能有一个当前状态。每次状态变化都要同步上面的进度计数。每完成一个任务，都必须在同一次本地提交中包含代码、测试、失败矩阵、该行状态和本文件中的证据；不得先把状态改成完成再补实现，也不得只提交代码而不更新进度。

### 9.1 架构与治理（4 项）

| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| AV-01 | 内置浏览器 ADR 与基线切换 | 写清替代关系、版本策略、全新 Profile、人工接管和首发边界；同步 CLAUDE.md、产品规划、前后端架构、工程结构和本专项 Roadmap，不静默保留冲突基线 | — | ⬜ 未开始 |
...
| AV-04 | 专项 Roadmap 状态与证据门禁 | 建立自动检查：87 行任务完整、汇总计数一致、只有一个 RED/实现中任务、完成任务具备提交和证据；明确拒绝向旧 Roadmap 复制专项状态 | AV-01..AV-03 | ⬜ 未开始 |
...
| CQ-05 | 文档、SBOM 与专项 Roadmap 收口 | 更新用户帮助、架构、隐私、许可证、版本锁、故障排查和卸载说明；本专项 Roadmap 记录真实提交和证据后才能完成 | CQ-04 | ⬜ 未开始 |
...
5. 覆盖任务适用的失败矩阵和资源清理；
6. 同一提交更新代码、测试、本文件任务状态和证据，不修改旧 Roadmap；
...
- 所有真实状态、提交、RED/GREEN 和边界证据进入本专项 Roadmap；旧 Roadmap 保持只读。
