...
| EB-11 | 登录与 Session 接入 | 抖音扫码、登录状态探测、失效接管、注销和重启复用在全新内置 Profile 走正式入口；验证码仍人工处理。**2026-08-03 收口进展：**macOS 已签名 production 候选从正常页面完成安全注销、重新扫码、主动复查、正常退出及同一 Profile inode 重启复用，包内进程清零；当前源码 Developer ID 包的 3,186 文件内容审计通过，但 Apple 普通/无 S3 上传均在零 completed part 时超时，尚无 staple。Windows 正式 App 真实扫码按用户要求留待可配合时补验，故保持待验收，不提前改完成态 | EB-07,EB-09 | 🔍 待验收 |
