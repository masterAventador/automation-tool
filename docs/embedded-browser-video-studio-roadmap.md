...
| BM-16 | 确定性与正式包验收 | 同输入/版本/字体/资源生成稳定结果；一句话、无模型手工模板边界、12 套整体风格和全部 134 项逐项预览/渲染；验证首期无 URL 输入/抓取入口，并覆盖断网/VPN 缺失、低配置、并发、崩溃、休眠、磁盘和双平台包。**2026-07-30 macOS 目录扫面实测：133 项真实渲染、唯一静态叠层按设计排除，12 风格与 30 帧双跑确定性通过；Windows 正式包及其余平台/压力证据仍待补，不提前改完成态。** | BM-08,BM-11..BM-15,VF-04（不含后置 BM-09～BM-10） | 🔍 待验收 |
