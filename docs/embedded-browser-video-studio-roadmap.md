...
| EB-07 | Executor 启动协议迁移 | 复用 BrowserLaunchRequest 和现有线程归属；由 Rust 传已验证内置路径，移除运行时发现 fallback | EB-06 | ✅ 已完成 |
| EB-08 | 启动健康状态迁移 | 用“浏览器组件正常/损坏/版本不兼容”替代“未安装/未选择 Chrome/Edge”，提供安全修复提示 | EB-06 | ✅ 已完成 |
| EB-09 | 全新内置 Profile 契约 | 新根目录、权限、稳定路径、排他锁、首次创建、登录后复用和安全删除测试；没有迁移或旧版分支 | EB-06 | ✅ 已完成 |
| EB-10 | 删除生产浏览器选择链路 | 先搜索所有引用，再移除设置项、启动门禁和生产消费者；系统浏览器发现代码不得继续存在可达的用户入口 | EB-08,EB-09 | ✅ 已完成 |
| EB-11 | 登录与 Session 接入 | 抖音扫码、登录状态探测、失效接管、注销和重启复用在全新内置 Profile 走正式入口；验证码仍人工处理 | EB-07,EB-09 | 🔍 待验收 |
| EB-12 | 搜索、浏览与候选提取原样迁移 | 现有固定 Playwright 页面对象和语义不重写，只换内置 Chromium 启动来源；现有页面变化和遮罩失败矩阵不降级 | EB-11 | 🔍 待验收 |
...
| EB-16 | 首发安装包与签名 | macOS arm64、macOS x86_64、Windows x86_64 每个目标只打包对应的一套完整 Chromium；macOS 内层签名/公证、Windows 签名、首次安装/卸载、包体积、进程退出和残留资源。**2026-07-31 对 `main@4b04408` macOS arm64 生产包复验：App/DMG Developer ID 严格签名、双公证票据、装订和 Gatekeeper 均通过；DMG 548,116,370 bytes，SHA-256 `c741bf93883cb5b35a8f1d12044498965b5cac98449cffb65c36b7e35f1b9ae4`。同一正式包又完成只读挂载、隔离安装、Rust 启动门禁、包内 Chromium 149.0.7827.55 离线真启动、正常入口可见 App health/version 请求、退出、卸载及容器/挂载/端口零残留。历史 ad-hoc 与“只验包不启动”缺口已关闭；macOS x86_64、Windows Authenticode/SmartScreen 与 Windows 交互式 App 启动仍待补。** | EB-15 | 🔍 待验收 |
| EB-17 | 无浏览器干净机纵向验收 | macOS 和 Windows 都在未安装 Chrome/Edge 的全新环境，从 App 正式入口完成安装、RPA 启动、扫码、搜索、受控动作、Browser Use 隔离演示和动效渲染；运行时不下载第二套浏览器且无残留。**2026-07-31 当前 macOS 签名公证发布候选以默认入口复验：先扫描 2,666 个包内文件并逐文件核验 Chromium 发行物，再在确实安装 Google Chrome 的宿主离线启动包内 149.0.7827.55，系统安装位置与浏览器缓存无新增、结束零残留。EB-17/PB-08 过期的 `.local/eb-16/run` 默认已统一到生产 `.local/release`。Windows 正式包、真正干净机与账号路径仍待补。** | EB-16,BU-07 | 🔍 待验收 |
...
| BU-02 | 单一 Chromium 双模式适配 | Rust 只下发已验证 `executable_path`；独立模式用临时 Profile，抖音模式用随机 loopback `cdp_url` 接管运营进程；关闭系统浏览器发现、默认安装/下载、Cloud 和 fallback | BU-01,EB-02 | ✅ 已完成 |
...
| IM-04 | 文案模型服务设置适配 | 把上游 LLM Provider、模型、Base URL 和连接测试映射到 App“文案模型服务”；密钥不进 React、配置和普通日志 | IM-03,VF-05 | ✅ 已完成 |
...
| BM-02 | 隔离 Node 22 Worker | Tauri sidecar 打包命令行/SDK 适配器，验证版本握手、无全局 Node 前置、取消、退出和错误脱敏 | BM-01,VF-02 | ✅ 已完成 |
| BM-03 | 共用 Chromium 渲染适配 | 只接收 Rust 已验证的单一完整 Chromium 路径，以独立无头进程和 RenderJob 临时目录启动；显式设置实际 Chromium major，禁用上游下载、系统浏览器发现和缓存 fallback | BM-02,EB-02 | ✅ 已完成 |
| BM-04 | HTML 渲染安全沙箱 | 私有目录、默认断网、本地资源、导航/下载/file URL 拦截、CPU/内存/帧数/大小/时长限制和 prompt injection 测试 | BM-02,BM-03,VF-03 | ✅ 已完成 |
| BM-05 | AI 一句话到动效视频编排 | 受限 MotionAuthoringAgent 读取锁定 workflow，以封闭工具把 Brief→DESIGN/脚本/分镜→可 seek 的 HTML/动画→lint/check/snapshot→局部修正→提交 RenderJob；逐帧 Chromium/FFmpeg 不调用模型，Agent 无 Shell/任意文件/Profile/密钥权限，未配置模型时自动制作明确不可用 | BM-04,VF-01,VF-05 | ✅ 已完成 |
| BM-06 | 12 套风格目录与中文选择器 | 全部 12 套有预览、中文名、适用场景、标签和键盘可访问性；少一套、重复或混入未公开模板都失败 | BM-01,VF-07 | ✅ 已完成 |
| BM-07 | 风格推荐、品牌微调与冻结 | 推荐 2～3 套但可看全量；品牌色/字体/Logo 覆盖、实际内容预览、frame.md 校验、版本/摘要冻结和当前 RenderJob 重现 | BM-05,BM-06 | 🔍 待验收 |
| BM-08 | App 原生编辑、Studio 精修与 Artifact 导入 | 不暴露上游 Studio 品牌；App 内编辑脚本/分镜/风格、播放预览、看进度、取消和管理成片；无模型时允许从固定模板替换声明变量/素材并手工精修，但不伪装成一句话生成 | BM-05,BM-07,VF-06 | 🔍 待验收 |
...
| BM-13 | 示例资产去品牌与原创替换 | Image Generation 生成原创头像/壁纸/插画/纹理；地图、3D、音效、字体和 SVG/UI 分别用有明确再分发权的资源或自研中性版本替换；建立资产来源、审核和摘要台账。**承接方约定（2026-07-23）：本任务由用户指定 codex 承接，其他 AI 会话不得自动激活；待替换清单见 `contracts/quality/motion-catalog-rights.v1.json` 与 `docs/development/BM-12.md` 的 pendingAssetReplacement（70 项素材 + 68 项商标指示词）。2026-07-30 运行时收尾：中性玻璃 API 与纹理项相对路径已进入精确发布重写契约，三个目标真实 Chromium 渲染通过，独立渲染豁免 19→16；证据见 `docs/development/BM-13.md`。** | BM-11,AV-02 | ✅ 已完成 |
| BM-14 | 134 项离线目录构建器 | 将上游代码、离线依赖和 App 资产 overlay 合成为只读版本化目录；不回写 submodule，构建可复现，缺项、远程引用、未登记资产或摘要漂移均失败。**2026-07-30 收尾：发布合成层精确物化 12 个上游宿主 template，并为 Liquid/Portal/Shatter 补齐确定性内容回退；锁定 Chromium 逐项均为 4 帧 4 摘要，独立渲染豁免 16→4→1；证据见 `docs/development/BM-14.md`。** | BM-12,BM-13 | ✅ 已完成 |
| BM-15 | 134 项自动选用与高级界面 | AI 可按分镜自动选；用户按中文分类查看全部 134 项离线预览并覆盖；每项显示性能、设备要求、适用和来源状态，不把零件叫作整体风格 | BM-08,BM-14 | 🔍 待验收 |
| BM-16 | 确定性与正式包验收 | 同输入/版本/字体/资源生成稳定结果；一句话、无模型手工模板边界、12 套整体风格和全部 134 项逐项预览/渲染；验证首期无 URL 输入/抓取入口，并覆盖断网/VPN 缺失、低配置、并发、崩溃、休眠、磁盘和双平台包。**2026-07-31 macOS 内容级与正式包边界收口：134/134 均保留非空代表帧与像素事实，133 项真实运动、唯一静态叠层按设计排除；25 个 component 使用 16 个多点文档宿主 + 9 个受控片段宿主，生成 11 张中文分类联系表并人工抽查；12 风格、30 帧双跑确定性及异步 3D 时间轴回归通过。`main@4b04408` 的 Developer ID DMG/App 又通过双票据、Gatekeeper、卷内六类资源与整包负面审计。Windows 正式包、全新普通用户安装后同路径及其余平台/压力证据仍待补，不提前改完成态。** | BM-08,BM-11..BM-15,VF-04（不含后置 BM-09～BM-10） | 🔍 待验收 |
...
| PB-05 | 抖音 Browser Use 发布前流程 | 用持久运营 Profile 和独占页面租约打开当前发布页；Browser Use 识别页面、选择/上传 Artifact、填写内容并停在提交前，覆盖改版/遮罩/登录失效/验证码 | BU-06,EB-11,VF-03 | 🔍 待验收 |
| PB-06 | 抖音单次发布与独立验收 | 在发布临界点展示目标账号、视频摘要和文案并确认；ActionGate + ledger 后最多点击一次，使用页面/作品列表等独立证据验收，不明确即 `outcome_uncertain` | PB-05 | 🔍 待验收 |
| PB-07 | 双平台发布界面与审计 | 发布页只展示 B站/抖音；平台适用性、授权、进度、审批、取消、失败、结果和审计使用统一 DTO，不暴露上游技术名；B站未配置凭据时显示可理解的“待配置”，不让整个发布模块启动失败。**2026-07-31 更正生产装配缺口：设置页受保护凭据入口、Rust 正式 API 分派、随包 ffprobe 真时长、Control Plane prepare/流式 upload/submission/session cancel、SQL 持久状态与对账、令牌提前续期、用户审批后单次提交、正常提交或取消后会话销毁（通信失败由两小时硬期限兜底）、OpenAPI/DTO 正式快照及容器契约资源均已接通；确定性 B站/API/App factory 403 项、Rust lib 136 项、前端 638 项全绿，生产镜像真实构建通过。无真实 B站凭据，真实投稿仍归 PB-08，不提前改完成态。** | PB-04,PB-06 | 🔍 待验收 |
| PB-08 | 分平台真实账号纵向验收 | macOS/Windows 正式 App 完成抖音可见 Browser Use 投稿；B站有凭据时完成 API 投稿，无凭据时保留独立 `待凭据验收` 检查项并继续后续任务。逐平台验证单一 Chromium、单次副作用、结果对账、退出清理和风控边界，禁止把 Mock 当真实平台证据。**2026-07-31 当前 macOS 签名公证发布候选先扫描 2,666 个包内文件，再用包内 Chromium 跑发布预检、单次副作用、结果对账、崩溃恢复与风控矩阵，33 项在 94.26 秒内全过且零残留。runner 已改用发布装配统一资源根与逐文件验证，补齐 Windows 安装树路径的实现缺口；Windows 正式包实跑和双平台真实账号投稿仍待补。** | PB-07,EB-17 | 🔍 待验收 |
...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| CQ-01 | 普通用户可理解性验收 | 用户不看技术文档也能选对两种制作方式，分清 12 套整体风格、134 个动效零件和独立视频剪辑模块；界面无未解释行业词。**2026-07-31 最新 `main` 刷新：隔离 PostgreSQL/正式迁移下的 macOS Tauri/WebKit 正常导航走完 8 页，术语门禁 34 项、视频工作台 54 项、真实 App 5/5 全绿，结束零残留；Windows 与正式安装包文案证据仍待补。** | IM-08,BM-16,VE-08 | 🔍 待验收 |
| CQ-02 | 用户界面上游名称泄漏扫描 | 扫描 React、WebView、标题、可访问性树、加载、错误、日志、任务、文件名和导出；法务页单独白名单。**2026-07-31 补齐真实 App：Tauri/WebKit 从正常导航走完 8 页，WebDriver 标准 computed role/label 共 410 条且独立复核无上游名；Harness 的发布临界点、不明确终态与启动诊断 4 场景保持全绿。** | IM-08,BM-16,VE-08,AV-03 | ✅ 已完成 |
| CQ-03 | 资源与失败矩阵联合压力测试 | 两种视频任务、独立剪辑任务、Browser Use、双平台发布与 RPA 共用 Chromium/本机资源并发时覆盖 CPU、内存、磁盘、网络、取消、紧停、崩溃、休眠和 App 退出；进程/Profile 互不干扰；B站无凭据时使用 Mock/契约路径并保留待补真实证据。**2026-07-31 在包含 T4 剪辑生产链的 `main@4b04408` Developer ID 公证正式包上刷新浏览器生命周期并发层：包内同一 Chromium 三线并发，用户取消只停 Browser Use，渲染硬崩溃不影响运营与 Browser Use，全局紧停和 App 退出均清空三棵进程树；运营 Profile 逐字节未被另外两线触碰，结束无残留。真实视频/剪辑/发布任务级资源压力、系统休眠与 Windows 证据仍待补。** | EB-15,BU-07,IM-08,BM-16,VE-08,PB-08 | 🔍 待验收 |
| CQ-04 | macOS/Windows 正式包纵向验收 | 从全新安装的真实 App 一句话创建两类视频、送入独立阿里云剪辑、预览、成片入库，再用抖音 Browser Use 完成真实发布；B站有凭据则同时真实发布，无凭据则核对完整实现、Mock/契约证据和 `待凭据验收` 状态，不阻塞本任务其余验收。**2026-07-31 当前 macOS 正式包可达段复验：EB-17 单一浏览器、CQ-03 三线生命周期、PB-08 33 项发布矩阵与 78 项激活台账扫描均通过；终验明确报告抖音账号缺失。T4 生产链已闭合：Tauri 持久化作业、统一 Artifact 私有 staging、已验签 Executor、OSS/IMS Provider、对账、回流、清理与 App 预览均已装配并通过 Mock 纵向；App 重启自动重新验证保留 Workspace，并按持久 vendor JobId 续查而不重复上传或 Submit。真实网关复跑在 Submit 前以 `InvalidAccessKeyId / AuthenticationRejected` 拒绝当前凭据，未产生剪辑计费；包含该链的 `main@4b04408` Developer ID 公证包已重建并通过隔离安装、正常入口启动和卸载，仍待有效凭据下的 macOS 云剪辑用户路径、真实发布账号、全新机器与 Windows 终验。** | PB-08,CQ-02,CQ-03 | 🔍 待验收 |
| CQ-05 | 文档、SBOM 与专项台账收口 | 更新用户帮助、架构、隐私、许可证、版本锁、故障排查和卸载说明；Roadmap 状态与 87 个独立证据文件一致后才能完成。**2026-07-31 补齐主架构文档中的浏览器页面租约与两种视频制作执行链，并以章节级门禁锁定；共享证据目录按当前 Roadmap 任务族双向核对，87 行 / 78 个已激活证据一致。仍待 CQ-04、真实用户走查和 Windows 卸载变体验收。** | CQ-04 | 🔍 待验收 |
