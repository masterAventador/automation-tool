...
| EB-11 | 登录与 Session 接入 | 抖音扫码、登录状态探测、失效接管、注销和重启复用在全新内置 Profile 走正式入口；验证码仍人工处理。**2026-08-02 当前用户故障已闭环：**修复执行器重连反复重放过期 durable outbox 导致控制面 4406 断开，并把 OS Profile 租约移出 Chromium user-data-dir；隔离 production-mode App 从“账号与平台”真实拉起内置 Chromium，主动复查刷新页面时间和线上健康 revision，macOS/Windows 原生租约矩阵均通过。新的正式签名包、安全注销、重新扫码与 Windows 正式 App 全生命周期仍待补，不提前改完成态 | EB-07,EB-09 | 🔍 待验收 |
...
| CQ-02 | 用户界面上游名称泄漏扫描 | 扫描 React、WebView、标题、可访问性树、加载、错误、日志、任务、文件名和导出；法务页单独白名单 | IM-08,BM-16,VE-08,AV-03 | 🔍 待验收 |
| CQ-03 | 资源与失败矩阵联合压力测试 | 两种视频任务、独立剪辑任务、Browser Use、双平台发布与 RPA 共用 Chromium/本机资源并发时覆盖 CPU、内存、磁盘、网络、取消、紧停、崩溃、休眠和 App 退出；进程/Profile 互不干扰；B站无凭据时使用 Mock/契约路径并保留待补真实证据 | EB-15,BU-07,IM-08,BM-16,VE-08,PB-08 | 🔍 待验收 |
...
| CQ-05 | 文档、SBOM 与专项台账收口 | 更新用户帮助、架构、隐私、许可证、版本锁、故障排查和卸载说明；Roadmap 状态与 87 个独立证据文件一致后才能完成 | CQ-04 | 🔍 待验收 |
