...
| BM-14 | 134 项离线目录构建器 | 将上游代码、离线依赖和 App 资产 overlay 合成为只读版本化目录；不回写 submodule，构建可复现，缺项、远程引用、未登记资产或摘要漂移均失败。**2026-07-30 收尾：发布合成层精确物化 12 个上游宿主 template，真实 Chromium 逐项 4 帧全部运动，独立渲染豁免 16→4；证据见 `docs/development/BM-14.md`。** | BM-12,BM-13 | ✅ 已完成 |
| BM-15 | 134 项自动选用与高级界面 | AI 可按分镜自动选；用户按中文分类查看全部 134 项离线预览并覆盖；每项显示性能、设备要求、适用和来源状态，不把零件叫作整体风格 | BM-08,BM-14 | 🔍 待验收 |
| BM-16 | 确定性与正式包验收 | 同输入/版本/字体/资源生成稳定结果；一句话、无模型手工模板边界、12 套整体风格和全部 134 项逐项预览/渲染；验证首期无 URL 输入/抓取入口，并覆盖断网/VPN 缺失、低配置、并发、崩溃、休眠、磁盘和双平台包 | BM-08,BM-11..BM-15,VF-04（不含后置 BM-09～BM-10） | 🔍 待验收 |
