...
H8-02 继续只修改 `executor/command_processor.py` 与 `executor/ledger.py`：普通 cancel 与 pause 共用持久控制推进器和 dispatch 门，账本根据同一 Attempt 的 verified/uncertain 副作用事实选择 cancelled/outcome_uncertain，不新增 cancel service、线程内存状态或 SQLite 表。`scripts/run_h8_02_acceptance.py` 复用 T3-14 的隐藏 task-termination App/Rust Command 和既有隔离脚手架，真实 Executor 只负责普通 cancel；原 spec 的 emergency-stop 半程由既有 FakeExecutor 收尾，H8-03 再接入离线 latch 与完整进程树停止。
