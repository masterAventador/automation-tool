...
1. **生成视频那一段（环节 5–8）已通，T58a / T58 关闭。** 一句话生成在真实 App 里跑通：
   3 分 34 秒，成片 12.000 秒 / 360 帧 / h264，并在 App 内播放成功
   （`scripts/run_t36_acceptance.py` 通过，证据
   `.local/embedded-browser-video-studio/t36-evidence/t36-one-sentence.mp4`）。
   两个此前的结论被实测推翻，见 `docs/development/T36-oneshot-video.md`：
   「15 分钟」是验收驱动 `waitUntil` 自己的预算，不是产品耗时；
   子进程非零退出的原因是**验收装的是一个过期的执行器包**，产品代码是好的。
2. **[T68] 产品账号登录**仍是工作台之前的阻塞（环节 1.5，另一条线在跟），
   它挡的是「进不进得去」，不是「生成得出来」。
3. **[T10] 从正式包跑一遍** — 上面那次验收跑的是 `control-plane-e2e` 的 debug 构建，
   **不是用户拿到的签名包**；P0-0 的新包出完之后必须在包上再走一遍同一条用户路径。
3. **[T61] setup 失败即 abort 的风险面** — 只调研不改。演示机是全新 Mac，setup 内任何意外失败客户看到的都是闪退零提示。拿到风险面再定要不要在演示前动核心启动路径。
4. 上面几条之外的任何新发现，**记进本文件的冻结区，不要派线去做**。Demo 前不再发散。
...
>
> 生成视频那一段（环节 5–8）已在真实 App 用户路径上实证走通。剩下的断点是环节 1.5 的产品账号登录，
> 以及「在用户真正拿到的那个签名包上再走一遍」。

> **演示形态已定：走品牌动效成片（hyperframes）那条**，不走素材成片。理由：外部依赖 1 个 vs 4 个串联；生成侧已实测产出会动的 mp4 而素材线一次都没走通过；渲染全在本机无现场网络风险；不需要用户去申请任何东西。代价是成片风格是文字动效/数据可视化，不是带实拍素材的口播视频。

| 序 | 环节 | 状态 | 证据 |
|---|---|---|---|
| 1 | 装上包、Gatekeeper 放行 | ✅ | 带 quarantine 判 `accepted / Notarized Developer ID`，用户已装到 `/Applications` |
| 1.5 | **产品账号登录** | ❌ **新阻塞，排在最前** | 见 T68。**登不进去就到不了工作台**，一句话生成无从谈起 |
...
| 5 | 点击提交 → 命令开始执行 | ✅ | 真实 App：点击到提交落地 3 秒。**「命令没开始」的旧结论已作废**——它一直立刻开始，只是当时占着主线程，而且几秒就失败了 |
| 6 | 编排子进程跑完 → 拿到合成 | ✅ | 真实 App：编排 178 秒返回合成。旧的非零退出是**验收装了一个过期执行器包**，不是产品缺陷 |
...
**生成视频这一段（5–8）已经全部实证走通**，证据在 `docs/development/T36-oneshot-video.md`。
剩下的 P0 阻塞是环节 1.5 的产品账号登录（T68，另一条线），以及在**正式签名包**上重跑同一条路径（T10）。
...
| T36 | 整条链路收口 | ✅ | 真实 App 用户路径通过，3m34s，成片 360 帧 / 12.000 秒并在 App 内播放。证据见 `docs/development/T36-oneshot-video.md` |
