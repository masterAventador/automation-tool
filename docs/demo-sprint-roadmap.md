...
| **小计：已收口** | **62** |
| 一、Demo 前必须收口 | 9 |
| T73～T100（T10 那轮挖出的新任务） | 15 |
| 冻结区·今晚撞见的技术债 | 12 |
| 冻结区·原有待办 | 10 |
| **小计：未收口** | **46** |
| **去重后总计** | **108** |

**T10 那一轮把任务从 74 涨到 108**——三十四条新的全部来自「真的去跑一遍」，其中 T80/T81/T82 已在同一轮修完合并，T83 已查证结论。

**Demo 前要收口的是 9 项**，其余 36 项冻结或已派给 codex（见 `docs/codex-parallel-batch.md`）。
...
| T91b | **渲染阶段失败且用户不在这一页仍然静默** | 🚧 | `render-visibility` | T91 交付时自己登记的遗留：轮询只在 `VideoStudio` 挂载时跑，切页就停；此时 store 里是 info 消息、侧边栏是蓝点「进行中」，渲染真失败了它也不知道。窗口约 10 秒（渲染阶段），比已修掉的编排阶段（2–3 分钟）小得多。修它要把轮询搬到共享外壳 |
| T88b | **屏效清单剩余项** | 🚧 | `ui-backlog` | `docs/development/T88-ui-density-sweep.md` 十四条里已处理五条，其余按三类分流：客观错的直接修、需审美判断的留给用户、标「读代码推断」的先实测——**T94 已经证明清单里至少有一条推断是错的** |
| T99 | **antd 折叠控件把英文状态词吞进可访问名** | ⬜ | **我** | T95b 写键盘用例时发现：antd 默认展开图标带 `aria-label="collapsed"`，被按内容计算的可访问名吸收，Chromium 实际暴露 `button "collapsed 智能素材成片的详细说明"`——**中文标签里夹一个英文状态词，而状态已由 `aria-expanded` 表达**。视频制作那处已随 T95b 修掉（改自绘 `aria-hidden` 箭头）。**同一缺陷在 `features/legal/third-party-software/ThirdPartySoftwareNotice.tsx` 上仍在**（实测 `button "collapsed MIT 许可证全文（英文原文）"`），改法同样是 `expandIcon` 一行。另：Playwright 的 `getByRole` **默认按子串匹配**，所以第一版用例是绿的——这类断言要 `exact: true` 才有牙 |
| T100 | **视频剪辑页在生产尺寸下只差 7px 放不下整页** | ⬜ | **我** | T96 对齐基准后实测：1280×800 下整页比视窗高 7px，导致出现滚动条（720 时需滚 87px）。**为了 7px 让整页出现滚动条**，是调一处 margin 就能消掉的距离。T96 按边界没碰产品代码 |
