...
| **小计：已收口** | **59** |
| 一、Demo 前必须收口 | 9 |
| T73～T96（T10 那轮挖出的新任务） | 14 |
| 冻结区·今晚撞见的技术债 | 12 |
| 冻结区·原有待办 | 10 |
| **小计：未收口** | **45** |
| **去重后总计** | **104** |

**T10 那一轮把任务从 74 涨到 104**——三十条新的全部来自「真的去跑一遍」，其中 T80/T81/T82 已在同一轮修完合并，T83 已查证结论。
...
| T96 | **所有 Playwright 用例跑在 720 高的视窗里，而注释声称 800** | ⬜ | **我** | `frontend/playwright.config.ts:16` 顶层写 `viewport: {1280, 800}`，但 `:21` 的唯一 project 铺了 `use: {...devices["Desktop Chrome"]}`，**project 级 `use` 覆盖顶层**，而该预设自带 1280×720。于是 `e2e/` 下每份用例都在比声称矮 80px 的视窗里跑——**包括 T88 那份文件头明确写着「1280x800，即生产窗口默认尺寸」的 `screen-density.spec.ts`**。方向偏严所以没掩盖过缺陷（测出的问题都是真的），但**所有像素结论的基准都不是它自称的那个**。T95 没改共享配置（拉长视窗会同时放松其它每份用例的折叠线，而当时多条线在改前端），只在自己文件里 `test.use` 显式钉住。收口需要统一基准并复核已有断言 |
