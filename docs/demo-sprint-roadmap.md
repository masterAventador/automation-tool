...
| T109 | 抖音「重新检查」两个按钮一点必报错 | 🚧 | ✅ 可 | **用户在正式签名包上实测报的**：退出再打开，被动展示的登录态是对的（仍显示「登录正常」），但点「打开登录处理」或「我已处理，重新检查」**必定**弹「暂时无法读取抖音登录状态，请稍后重试。」。根因未知，因为 `PlatformSessions.tsx` 的 `catch { setFailure(true); }` **把真实错误整个丢掉**，只留一个布尔——代码把答案扔了。已派线（`douyin-recheck`）：先拿证据定根因，再把吞错误这件事本身修掉。抖音是演示主链路，算演示阻塞 |
