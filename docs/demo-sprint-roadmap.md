...
| T127 | **出包死在最后一步：`bundle/dmg/` 父目录没人建**（我引入的，**等用户授权后一行可修**） | 🚧 | ❌ 不可 | 07-27 08:22–08:47 连续三次出包，**第一次和第三次同一原因**：`create_disk_image` 的 `tempfile.TemporaryDirectory(dir=output.parent)` 要求 `bundle/dmg/` 已存在，而 `fill_disk_image` 里那句 `output.parent.mkdir(parents=True, exist_ok=True)` **在它之后才执行**。老写法能过是因为 `hdiutil -srcfolder` 自己会创建输出目录；我 07-27 改成「建空映像 + 自己 ditto 填充」（T10 那条 hdiutil 修复）之后，没人负责建那个父目录。`tauri build --bundles app` 只产出 `macos/`，**Tauri 的 DMG 打包器根本不执行**，所以 `dmg/` 永远不会被构建创建。**手工建目录已实测无效**：08:31 建的，`tauri build` 在 08:36:41 重建整个 `bundle/` 时抹掉了。**修法**：把那句 `mkdir` 挪到 `TemporaryDirectory` 之前（或在 `create_disk_image` 入口处建）——不新增逻辑、不改行为。**当前状态**：已签名并公证装订的 `.app` 在 `wt/release/.local/release/cargo-target/release/bundle/macos/`，票据 `validate worked`，**只差装进 DMG 这一步**。用户 08:25 交代「别动代码」后去上班，故未改，等授权 |
