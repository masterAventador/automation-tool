...
| T119 | **Windows 上出不了包：没有可用的 C 编译器** | 🚧 | T116 实测。`run_eb_16_windows_acceptance.py:557` 必过 `prepare_video_runtime(platform="windows")`，它在 winbox 上失败。**真实原因被日志截断吞掉**——`[-8:]` 只留最后 8 行而 curl 进度条把它顶满了；直接跑 builder 才挖出来：**`No working C compiler found.`**，另外 `bash` 也不在 PATH（`C:\Program Files\Git\bin\bash.exe` 存在但没进 PATH）。media-toolchain 要从源码编 ffmpeg，需要 POSIX 风格工具链（MSYS2 之类），而机器上装的是 MSVC。**这决定了 T7 能做到哪一步**：Windows 的 GUI 验收要在正式包上做，而正式包出不来。**入账时我把它写成「要不要装工具链，等你定」，这个判断是错的**——T116 记录里就有线索：那台机器 `.local\vf04-msys2` 目录还在、安装器 `msys2-base-x86_64-20260611.sfx.exe` 还在、还有一份 07-24 构建出来的 `ffmpeg.exe`（19,269,632 字节）。**它本来就是这么装的，是环境退化了，不是从没有过**，所以这不是一个需要用户拍板的新决定。已派线 `win-toolchain` 去恢复（T123），并明令**不许为了省掉重建去改缓存键或把那份旧产物塞进当前缓存冒充新的**——那正是刚修完的 T111，在这里重演性质更严重 |
