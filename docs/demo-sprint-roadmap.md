...
| T125 | **desktop-e2e 六个验收自 07-26 21:16 起整族跑不起来** | 🚧 | ✅ 可 | **T57b 收尾时撞出来的，与它自己的任务无关。** `fb6d122`「让桌面验收加载生产组合入口」把 `test-tauri-main.tsx` 从自建 composition root 改成 `void import("./main")`，启动检查从**无条件返回 ready** 的测试桩换成生产的 `createDesktopStartupCheck`——**方向是对的**（正是 `single_build_path.rs` 要消除的测试构建分叉，旧写法让 workbench 断言恒真），**缺的是把六个 runner 接进 `scripts/desktop_e2e_prerequisites.py`**。**主线独立复核**：`run_h8_20/21/22_macos/22_windows` 四个驱动 `prepare_startup_gate` 命中数**全是 0**，而正常的 `run_t36_acceptance.py` 是 2；`test-tauri-main.tsx` 现在确实只有三行、加载生产入口。子代理实跑 `run_h8_20` 停在 spec **第一行**（`Expected: "RPA 运营工作台" / Received: "桌面运行环境需要处理"`），把 spec 换回 HEAD 版复跑得到**逐字相同**的失败——与 spec 内容无关。受影响：`update-download`、`update-ui`、`update-macos-package`、`update-windows-package`、`diagnostic-export`、`model-service`。**`desktop-e2e-remaining-specs.md` 记的「07-26 全绿」是那次提交之前的事。** 与 T117 同一形状：一次正确的改动静默打断了一族没人跑的驱动，而它们不在任何门禁里（T79 的归属门禁刚建好，但补前置是另一件事） |
