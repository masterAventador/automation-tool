...
| T114 | **「已登录 → 重启 → 点重新检查」这条路径从无覆盖** | 🚧 | ✅ 可 | **正是用户实测撞到的那条**。T109 自报：`e2e-tauri/platform-session.spec.ts` 确实点这两个按钮，但**从未登录态起步、只走到 `awaiting_scan`，从不进入 healthy 后重启再点**。同一个 spec 里还留着一句注释写明页面会把注销失败吞成通用文案——**有人在 E2E 里绕过缺陷却没回来修页面**（T99 修过同形状的另一处）。难点不在写用例，在于**能否不接真实账号而确定性地造出「已登录」**；其中最有价值的可能是播种那个导致按钮永久失败的 `active` 租约标记——它不需要任何登录态。**结论若是「不接真实账号无法有意义地覆盖」也算有效交付**。已派线 `douyin-restart-e2e` |
