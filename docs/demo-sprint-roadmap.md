...
| T111 | **Worker 源码改了不会进包**（缓存键漏掉源码） | 🚧 | ✅ 可 | T108 直接对比两个产物查实：**已安装正式包里的 Worker** → `stylesheetLoaded: false`、`bgm_volume_select` 仍可见、无提示；**当前构建缓存里的 Worker** → 全部正确。根因在 `scripts/video_runtime_cache.py:61` 的 `contract_fingerprint()`——它只摘要契约 JSON，**`workers/material_montage/*.py` 完全不参与缓存键**，所以改 Worker 源码不会让缓存失效，发版会静默复用旧二进制。这解释了 T32 为什么「提交在包里、效果不在」。**它会静默吞掉对 Worker 源码的任何修改**，也就是决定你在包里看到的到底是不是修好的版本，所以算演示阻塞。参照物已有：T74 修执行器包缓存键时用的就是逐文件摘要源码的做法 |
...
| ID | 任务 | 并行 | 一句话 |
|---|---|---|---|
| T110 | 取消的两处小口子（机制本身已验证可用） | 🤖 | T105 复核时读代码查实：**取消确实会停掉渲染**——`worker.mjs:1124` 逐帧 `access()` 那个标记文件，命中就 `finish({status:"cancelled"})`，所以子代理登记的「没验证是否真停」现在有答案了。剩两处：① `motion_video_studio.rs:1366` 写完标记**立刻**把快照推到 `Cancelled`，不等执行器确认，而 `CLAUDE.md` §4.4 要求「只有执行器确认终态后才能宣称副作用已经停止」——实际窗口是一帧，不挡演示；② 标记文件名 `.automation-tool-cancel` 在 `motion_video_studio.rs:22` 和 `worker.mjs:81` **各写了一份字面量**，没有单一来源，改一处不会带动另一处 |
...
| T105 | **取消之后侧边栏不再替一条已经不存在的运行说话**（`39b8d07`）。**「单槽 message 要不要改结构」这个前提被推翻**：能表达「哪条任务、什么终态」的结构 T101 就加好了（`ownJobs[].outcome`），缺的是侧边栏一直没去读它。`motionRunAttention` 的 in-flight 判据 `pending !== null \|\| message !== null` 是 T91 那个布尔函数的原样继承，写它时 store 里确实没有别的东西记得住结果，T101 之后就过期了。改成 `motionRunNeedsWatch(current)`，**一行，没有新增 dismiss、没有新增结构，而且比原来更严**（原来「有条提示在」就算在跑）。**同一行修掉方向相反的第二张脸**：关掉页面上那条可关闭的 info 提示，会让一条还在跑、还在轮询的渲染从侧边栏整个消失——比取消那半更坏。`failed` 分支继续读 message 不是没收拾干净：编排失败时任务压根还不存在（T91 查证过快照写在编排返回之后），任何以任务为键的结构都装不下它。**主线复核**：独立复跑 41 条通过；变异验证把该行改回旧写法，**正好 3 条新用例红、38 条老用例绿**。剩余窗口另立 T110 |
