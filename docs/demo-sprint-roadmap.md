...
| T119 | **Windows 上出不了包：没有可用的 C 编译器** | 👤 决策 | T116 实测。`run_eb_16_windows_acceptance.py:557` 必过 `prepare_video_runtime(platform="windows")`，它在 winbox 上失败。**真实原因被日志截断吞掉**——`[-8:]` 只留最后 8 行而 curl 进度条把它顶满了；直接跑 builder 才挖出来：**`No working C compiler found.`**，另外 `bash` 也不在 PATH（`C:\Program Files\Git\bin\bash.exe` 存在但没进 PATH）。media-toolchain 要从源码编 ffmpeg，需要 POSIX 风格工具链（MSYS2 之类），而机器上装的是 MSVC。**这决定了 T7 能做到哪一步**：Windows 的 GUI 验收要在正式包上做，而正式包出不来。**要不要为此在 winbox 上装 MSYS2 工具链、还是把 Windows 验收推到 Demo 之后，等你定** |
...
| T122 | **`os.defpath` 在 Windows 上不是可用的最小 PATH，三处受影响** | 🚧 | ✅ 可 | **用意对、取值错**：三处都用 `env={"PATH": os.defpath}` 拿最小 PATH 跑冻结产物 / 构建，证明不依赖开发者环境——这是单一构建路径要守的性质，不该改掉。但 POSIX 上它是 `:/bin:/usr/bin`（合理），**Windows 上实测是 `.;C:\bin`（根本不存在）**，连系统 DLL 都解析不了，失败原因与被测性质无关。两处在 `test_pyinstaller_bundle.py`（测试，已判定为测试自身不可移植）；**第三处在 `build_material_video_worker_candidate.py`，是生产构建路径**——Windows 构建目前卡在更前面的 media-toolchain 缺 C 编译器（T119），所以这一处还没被撞到，**它是藏在后面的第二道**。已派线 `minimal-path` |
