...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取**真实音频时长**；支持用户上传录音替代（转写后对齐句子，复用 LE-14 ASR）。T1～T3 已完成且最终复审无 finding。T4 补齐 TTS 超时/空音频/零时长矩阵与真实百炼分句→TTS→随包 ffprobe 纵向验收；真实终态为 3 个音频、`3680/4720/5280 ms`。相关单元/驱动 258 条、整目录 integration 458 条、三个自有模块 604 条语句/214 分支 100%、Ruff 与严格 mypy 全绿，待 T4 Codex Review，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T4 实现检查点执行 Codex Review。** TTS 失败矩阵、真实百炼分句/TTS、随包
ffprobe 二次对账与模块收口门禁均已通过；三个 LE-15 自有模块语句/分支覆盖率均为
100%。复审无 finding 后把 LE-15 顶格标为 `🔍 待验收`，并进入 LE-16 T1 语义匹配。
