...
| LE-14 | 人声检测与转写 | 三级漏斗：`silencedetect` 判有无声音 → Silero VAD（本地 ONNX，约 2 MB）判是人声还是纯环境音 → 仅对人声素材调百炼语音识别转写（只传音轨不传视频，避免为筛选步骤打包 140 MB+ 的本地 Whisper）。产出 `has_speech`、`speech_segments_ms`、`speech_transcript` 写回 Material；**onnxruntime 与 VAD 模型必须进安装包并有出厂门禁核对**；纯音乐误判人声、人声判成环境音、转写为空、方言/嘈杂环境、ASR 超时、路人背景说话被当主体的失败矩阵。T1 Review finding 已修复，等待复审，证据见 `docs/development/LE-14.md` | LE-07 | 🚧 实现中 |
...
**LE-14 人声检测与转写 T1 修复复审。** 非法时长前置拒绝 finding 已补 RED 并修复；
复审通过后立即进入 T2。
