...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1 已完成；首轮 Review 的六项配置、资源与合法输入边界 finding 已按 TDD 修复，复审无 finding，证据见 `docs/development/LE-15.md`。下一步按 T2 先写逐句 TTS 与 ffprobe 真实时长 RED | LE-06,LE-14 | 🚧 实现中 |
