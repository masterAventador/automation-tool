...
**LE-16 T1、T2 已完成。** T2 只在绑定当前素材字节的实际可解码区间与镜头区间交集中
选择等长窗口，不以容器声明时长补帧；零可解码帧、线性区间求交、UUIDv4 边界和同分素材
输入顺序均已按三轮 Codex Review 收口。最终 T2 专项 52 条，选择器与协议模块语句/分支
覆盖 100%，受影响回归 189 条。下一步进入 T3，把有人声素材按输入顺序编排成自带旁白
段落；文案句子只分配给无人声素材。
