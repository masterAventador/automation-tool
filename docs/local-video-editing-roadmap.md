...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| LE-13 | 素材理解 | 抽帧送百炼多模态，产出描述、标签与镜头时间区间并写回 Material；**关闭深度思考**（`enable_thinking=false`，作为配置项非硬编码，见设计文档 §4.5）；超时/拒答/空描述/token 超限的失败矩阵；真实模型调用验收。T1 已完成并通过最终复审，T2 正在先定结构化结果与原子写回；证据见 `docs/development/LE-13.md` | LE-08 | 🚧 实现中 |
...
**LE-13 素材理解 T2。** 先定结构化结果与 Material 原子写回语义，再写失败测试。真实模型
