...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前四轮 Review 共一项 P1、四项 P2 已按 TDD 修复，最新一轮恢复接受与相邻句首尾重叠的精确短句，同时保留漏读中间句的跨边界拼接拒绝；43 条单元、1 条真实媒体、Ruff 与严格 mypy 全绿，待第五轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第四轮 finding 修复检查点执行 Codex Review。** 精确中间句不再因为恰好
等于前句后缀加后句前缀而误拒，存在识别误差的候选仍执行跨边界拼接检查。43 条 T3
单元、1 条真实媒体、Ruff 与严格 mypy 全绿。第五轮复审收口后进入 T4 失败矩阵、
真实验收与模块收口。
