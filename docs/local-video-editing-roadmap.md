...
| LE-22 | macOS 正式包纵向验收 | 从全新安装的正式 App 正常入口导入本地素材 → 一句话生成草稿 → 出片 → 成片入库并可播放；素材含至少一条有人声素材，核对它走的是原声而非 TTS；证据含 ffprobe 读数与产物尺寸。T1～T3 已完成；macOS arm64 真机从 DMG 隔离安装正式 App，经真实百炼理解/ASR 生成 `visual/ambient/caption` 时间线并发布唯一 Artifact；ffprobe 为 H.264 `720×1280@20`、102 帧、5.100 秒和 AAC，原声 PCM 包络相关性 `0.999996`，完整 integration `472 passed, 18 skipped`，证据见 `docs/development/LE-22.md` | LE-20,LE-21 | ✅ 已完成 |
| LE-23 | Windows 正式包纵向验收 | 同 LE-22，在 Windows 正式包上独立完成；核对随包 ffmpeg、VAD/ASR 运行时与字体在 Windows 包内真实存在。T1～T3 已完成；Windows 真机以非提升交互用户生成并安装 current-user NSIS，安装后通过浏览器、三项视频运行时、134 项动效、字体/Worker 门禁，并从正式页面完成真实百炼理解/ASR、原声时间线、渲染和 Artifact 发布；ffprobe 为 H.264 `720×1280@20`、102 帧、5.100 秒和 AAC，原声 PCM 包络相关性 `0.999996`，完整 integration `460 passed, 30 skipped`，随后卸载并清理注册表/端口，证据见 `docs/development/LE-23.md` | LE-22 | ✅ 已完成 |
...
**24 项任务已全部完成实现、逐任务 Review、功能验收及所需的 macOS/Windows 正式包纵向验收。**
下文保留 LE-10～LE-18 的推进背景，不再把其中“下一步进入 LE-17”或“保持待验收”的历史句子
解释为当前状态。
