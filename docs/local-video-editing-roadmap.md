...
**LE-15 T4 失败矩阵、真实验收与模块收口。** T3 第十一轮复审无 finding，已正式收口。
先把 TTS 超时、空音频、零时长等故障边界与真实调用判据【先定】进 LE-15 任务文档，
再按 RED → GREEN 完成失败矩阵；模块全量与真实百炼分句/TTS、随包 ffprobe 纵向验收只
在本次模块收口统一执行。
