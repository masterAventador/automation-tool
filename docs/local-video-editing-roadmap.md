...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前三轮 Review 共一项 P1、三项 P2 已按 TDD 修复，最新一轮把分区改为精确快路、带状距离、优先搜索与 1024 次硬预算，并拒绝从相邻句边界拼出未朗读句子；42 条单元、1 条真实媒体、191 条相关回归及 100% 新模块覆盖全绿，待第四轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第三轮 finding 修复检查点执行 Codex Review。** 分区已改为精确快路、
带状编辑距离、最接近句长优先的深度搜索、失败状态缓存与 1024 次候选硬预算；相邻句
后缀/前缀不能再拼成一条未朗读句。42 条 T3 单元、1 条真实媒体、191 条相关回归、
100% 新模块覆盖、Ruff 与严格 mypy 全绿。第四轮复审收口后进入 T4 失败矩阵、真实验收
与模块收口。
