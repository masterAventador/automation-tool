...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；32 条聚焦、161 条相关回归、100% 新模块覆盖及真实随包媒体验收全绿，待实现检查点 Codex Review，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 实现检查点执行 Codex Review。** 用户录音只走已批准纯音频、本地真实 VAD
边界和 LE-14 路径无关 ASR；32 条聚焦、161 条相关回归、100% 新模块覆盖、真实随包媒体、
Ruff 与严格 mypy 全绿。复审收口后进入 T4 失败矩阵、真实验收与模块收口。
