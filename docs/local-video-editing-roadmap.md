...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1 已完成且复审无 finding；T2 逐句 TTS、批次回滚、付费前预检与随包 ffprobe 真实时长实现检查点完成，72 条相关测试及静态门禁全绿，待 Codex Review，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
