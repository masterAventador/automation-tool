...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| LE-13 | 素材理解 | 抽帧送百炼多模态，产出描述、标签与镜头时间区间并写回 Material；**关闭深度思考**（`enable_thinking=false`，作为配置项非硬编码，见设计文档 §4.5）；超时/拒答/空描述/token 超限的失败矩阵；真实模型调用验收。T1、T2 已完成，T2 Review 的两条 P2 已修复；T3 已先定失败语义并进入 RED；证据见 `docs/development/LE-13.md` | LE-08 | 🚧 实现中 |
...
**LE-13 素材理解 T3 失败矩阵。** 逐条注入超时、显式拒答、空描述与 token 超限，确认
固定脱敏拒绝且不产生部分结果；完成本任务 Codex Review 后进入 T4。真实模型
调用受本机凭据约束，自动化分层实现不因此停下；
无凭据时登记补验收并继续后续独立任务。
