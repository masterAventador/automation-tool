...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前五轮 Review 共一项 P1、六项 P2 已按 TDD 修复，最新一轮只对整份精确转写走直接成功，非精确转写仍拒绝从相邻句边界拼出漏读句；44 条单元、1 条真实媒体、Ruff 与严格 mypy 全绿，待第六轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第五轮 finding 修复检查点执行 Codex Review。** 整份精确转写直接证明
全部句子，非精确转写的每条候选仍执行跨相邻句拼接检查；任务证据页顶部摘要已同步。
44 条 T3 单元、1 条真实媒体、Ruff 与严格 mypy 全绿。第六轮复审收口后进入 T4
失败矩阵、真实验收与模块收口。
