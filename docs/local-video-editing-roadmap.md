...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；首轮 Review 两项 P2 已修，第二轮 Review 的固定字符比例误拒合法句内漏字一项 P2 也已按 TDD 改为有界动态分区并补文本归属歧义拒绝，38 条单元、1 条真实媒体、187 条相关回归及 100% 新模块覆盖全绿，待第三轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第二轮 finding 修复检查点执行 Codex Review。** 固定字符比例已改为逐句
15% 长度范围内的有界动态分区，并拒绝取自另一条脚本句的归属歧义。38 条 T3 单元、
1 条真实媒体、187 条相关回归、100% 新模块覆盖、Ruff 与严格 mypy 全绿。第三轮复审
收口后进入 T4 失败矩阵、真实验收与模块收口。
