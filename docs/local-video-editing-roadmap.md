...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| LE-13 | 素材理解 | 抽帧送百炼多模态，产出描述、标签与镜头时间区间并写回 Material；**关闭深度思考**（`enable_thinking=false`，作为配置项非硬编码，见设计文档 §4.5）；超时/拒答/空描述/token 超限的失败矩阵；真实模型调用验收。T1～T3 已完成并通过各自最终复审；T4 已先定真实百炼、真实素材与 PostgreSQL 纵向边界并进入 RED；证据见 `docs/development/LE-13.md` | LE-08 | 🚧 实现中 |
