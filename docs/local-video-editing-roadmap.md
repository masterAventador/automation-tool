...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取**真实音频时长**；支持用户上传录音替代（转写后对齐句子，复用 LE-14 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前九轮 Review 共一项 P1、十四项 P2 已按 TDD 修复，最新一轮按实际 DP 行分配成本限制跨候选回溯，300/500 字符反例从约 6.6/14.9 秒收敛到约 1.76/1.80 秒。53 条单元、1 条真实媒体、100% 新模块覆盖、Ruff 与严格 mypy 全绿，待第十轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第九轮 finding 修复检查点执行 Codex Review。** 候选次数继续受 1,024
硬上限，普通带状距离和增量边界表另按实际完整行分配量累计扣减确定性预算；200 字符
回归先从 8,232,156 / 54,546,473 个单元格准确变红，修复后 200/300/500/1,000 字符
压力约在 1.75–1.86 秒安全拒绝。53 条 T3 单元、1 条真实媒体、100% 新模块覆盖、Ruff
与严格 mypy 全绿。第十轮复审收口后进入 T4 失败矩阵、真实验收与模块收口。
