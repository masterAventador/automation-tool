...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| LE-13 | 素材理解 | 抽帧送百炼多模态，产出描述、标签与镜头时间区间并写回 Material；**关闭深度思考**（`enable_thinking=false`，作为配置项非硬编码，见设计文档 §4.5）；超时/拒答/空描述/token 超限的失败矩阵；真实模型调用验收。T1 已按交接先定供应商无关协议、复用 `video_creative` 模型目录、显式注入配置与无路径 JPEG 输入；首次收集期因模块不存在 RED，最小实现后专项 `6 passed`，Ruff 与严格 mypy 通过，等待逐任务 Codex Review，证据见 `docs/development/LE-13.md` | LE-08 | 🚧 实现中 |
...
**LE-13 素材理解 T1。** 供应商无关 Adapter、锁定百炼图片模型目录、请求级
`enable_thinking` 和无路径 JPEG 输入已按 RED → GREEN 实现并通过专项门禁；下一步提交 T1
并执行逐任务 Codex Review，修复复审发现后再进入 T2 的结构化结果和 Material 原子写回。
真实模型调用受本机凭据约束，自动化分层实现不因此停下；无凭据时登记补验收并继续后续
独立任务。
