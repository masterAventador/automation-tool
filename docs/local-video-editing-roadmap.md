...
| LE-14 | 人声检测与转写 | 三级漏斗：`silencedetect` 判有无声音 → Silero VAD（本地 ONNX，约 2 MB）判是人声还是纯环境音 → 仅对人声素材调百炼语音识别转写（只传音轨不传视频，避免为筛选步骤打包 140 MB+ 的本地 Whisper）。产出 `has_speech`、`speech_segments_ms`、`speech_transcript` 写回 Material；**onnxruntime 与 VAD 模型必须进安装包并有出厂门禁核对**；纯音乐误判人声、人声判成环境音、转写为空、方言/嘈杂环境、ASR 超时、路人背景说话被当主体的失败矩阵。T1 一级短路实现与捕获力完成，等待本任务 Codex Review，证据见 `docs/development/LE-14.md` | LE-07 | 🚧 实现中 |
...
**LE-14 人声检测与转写 T1 Codex Review。** 一级短路实现与捕获力已完成；审查 finding
修复并同步台账后立即进入 T2。
