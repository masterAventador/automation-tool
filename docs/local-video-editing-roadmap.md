...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取**真实音频时长**；支持用户上传录音替代（转写后对齐句子，复用 LE-14 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前八轮 Review 共一项 P1、十三项 P2 已按 TDD 修复，最新一轮以两张增量带状 DP 表复用全部重叠边界距离，并同步锁定实际片段重新分配与替代总距离不更差的歧义拒绝语义；Review 的 1,000 字符压力探针恢复到 0.431 秒和 4 次编辑距离。52 条单元、1 条真实媒体、100% 新模块覆盖、Ruff 与严格 mypy 全绿，待第九轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第八轮 finding 修复检查点执行 Codex Review。** 重叠边界的替代距离改由
两张增量带状 DP 表一次计算并复用，Review 的 1,000 字符合法压力探针从 9.86 秒恢复到
0.431 秒、编辑距离调用从约 302 次降为 4 次；【先定】同步锁定实际片段重新分配、替代
总距离和歧义 fail closed 语义。52 条 T3 单元、1 条真实媒体、100% 新模块覆盖、Ruff
与严格 mypy 全绿。第九轮复审收口后进入 T4 失败矩阵、真实验收与模块收口。
