...
**LE-13 素材理解。** 前置 LE-08 已完成分层实现并顶格 `🔍 待验收`。下一步先按交接文档
建立 LE-13 任务证据，核对现有模型 Provider、配置与 Material 写回边界；随后严格按 RED →
GREEN 推进受控抽帧消费、百炼多模态请求、结构化描述/标签/镜头时间区间和失败矩阵。真实
模型调用受本机凭据约束，自动化分层实现不因此停下；无凭据时登记补验收并继续后续独立任务。
