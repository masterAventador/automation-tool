...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前六轮 Review 共一项 P1、九项 P2 已按 TDD 修复，最新一轮用前/中/后三个实际片段及截短相邻脚本距离证明重叠短句独立出现，同时保持重复边界漏句和高歧义调序 fail closed；47 条单元、1 条真实媒体、100% 新模块覆盖、Ruff 与严格 mypy 全绿，待第七轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第六轮 finding 修复检查点执行 Codex Review。** 重叠短句改由前/中/后三个
实际片段与截短相邻脚本距离共同证明，不再用整份精确文本跳过逐句歧义，也不因其他句的
一次允许误识别而误拒；重复边界借字同样拒绝。47 条 T3 单元、1 条真实媒体、100% 新
模块覆盖、Ruff 与严格 mypy 全绿。第七轮复审收口后进入 T4 失败矩阵、真实验收与模块
收口。
