...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1 已完成且复审无 finding；T2 首轮 Review 的三项 P2、一项 P3 已按 TDD 修复，批次回滚不再误删旧产物、父级冲突在付费前拒绝、固定异常不携带私有路径链；26 条聚焦测试达到 100% 覆盖，81 条相关回归全绿；第二轮代码复审无 finding，唯一台账指针 finding 已补正，待最终复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T2 finding 修复检查点执行最终 Codex Review。** 逐句 TTS 只复用既有合成器，
任何云调用前预检全部固定输出、父级目录与随包工具；每句时长只读 LE-07 的公开 ffprobe
边界，批次失败只回滚本批，不自动重试。26 条 T2 聚焦测试达到 100% 覆盖，81 条相关
测试、Ruff 与严格 mypy 全绿；第二轮代码复审无 finding，唯一台账指针 finding 已补正。
最终复审收口后进入 T3 用户录音转写与句子对齐。
