...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1/T2 已完成且最终复审无 finding；T3 复用 LE-14 有界 PCM/VAD/ASR，在付费前要求每句至少一个真实人声区间，只在真实 VAD 边界分组，不伪造 ASR 时间戳；前七轮 Review 共一项 P1、十一项 P2 已按 TDD 修复，最新一轮把疑似借出的中间句边界重新分配回左右实际片段，关闭 ASR 重复插入伪造漏句，并以长度下界和统一工作预算约束边界距离；51 条单元、1 条真实媒体、100% 新模块覆盖、Ruff 与严格 mypy 全绿，待第八轮复审，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 第七轮 finding 修复检查点执行 Codex Review。** 疑似借出的中间句边界改为
重新分配回左右实际片段后比较总距离，关闭允许的 ASR 重复插入伪造漏句；长重叠边界先
按长度下界剪枝，真正发生的替代距离计算消费统一工作预算。同一 1,000 字符压力探针由
约 6.62 秒降至 0.742 秒。51 条 T3 单元、1 条真实媒体、100% 新模块覆盖、Ruff 与严格
mypy 全绿。第八轮复审收口后进入 T4 失败矩阵、真实验收与模块收口。
