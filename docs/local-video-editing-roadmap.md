...
**LE-12 T1～T2 已完成；下一步进入 T3 持久作业账本、检查点与失败分类。** LE-10 已交付从真实领域投影、绝对
...
分类。现在进入 T3 的 App 私有持久作业账本、检查点和 `EditingJob` 状态映射。
