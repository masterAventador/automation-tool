...
| ID | 任务 | 交付与验收 | 依赖 | 当前状态 |
| --- | --- | --- | --- | --- |
| LE-13 | 素材理解 | 抽帧送百炼多模态，产出描述、标签与镜头时间区间并写回 Material；**关闭深度思考**（`enable_thinking=false`，作为配置项非硬编码，见设计文档 §4.5）；超时/拒答/空描述/token 超限的失败矩阵；真实模型调用验收。T1～T4 与真实纵向验收全部完成；单次 Codex Review 的一条 P1、两条 P2 已补 RED 并修复，证据见 `docs/development/LE-13.md` | LE-08 | ✅ 已完成 |
...
**LE-14 人声检测与转写 T1。** 先复用 LE-07 `MaterialFacts.has_audio` 建立三级漏斗首层，
无有效音频时不得加载 VAD 或调用 ASR。
