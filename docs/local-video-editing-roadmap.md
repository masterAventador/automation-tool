...
**LE-10 T1～T5 已完成；T6 进入受控执行与 ffprobe 核验 RED。**
...
不是硬切冒充。首轮 Review 进一步真实执行硬切/转场混合链，发现 concat 输出的
`1/1000000` timebase 无法进入下一步 `1/fps` xfade；修复为链内每次 concat 后显式恢复
帧时基，五段硬切→fade→硬切→dissolve 真实复验输出 20 帧、1.0s。完整 integration 仍留到
T7 收口时决定。修复提交经第二轮复审无新增 finding，T4 正式完成；T5 先建立无路径字幕
计划与领域投影，再让 LE-09 公开入口生成的透明 PNG 按同一帧栅格进入 overlay。T5 已完成
协议、领域投影、本地原子 PNG 绑定和 compiler 装配，LE-09/LE-10 受影响回归 471 条；真实
纵向输出 H.264、720×1280@30、30 帧、1.0s，窗外帧均匀、两个窗内帧均出现白/黑字面，
filter graph 不含字幕文本或本机路径。完整 integration 仍留到 T7 收口时决定。
T5 首轮 Review 实测公开 overlay set 可绕过上游接受 `1×1@1fps、1ms、513 条字幕`，且第二张
PNG 中断会遗留第一张；现已补齐协议同款边界、派生帧数、512 上限和 BaseException 清理，
同时增加三层字幕常量漂移守卫。修复后受影响回归 484 条、三层生产模块分支覆盖均 100%，
字幕跨越真实 hard-cut 的随包 FFmpeg 复验仍精确输出 30 帧、1.0s。
修复提交经第二轮复审无新增 finding，T5 正式完成；T6 开始把计划、素材快照、字幕出图、
限时 FFmpeg、ffprobe 形状核验、摘要与同目录原子发布收进一个失败不留半成品的执行入口。
