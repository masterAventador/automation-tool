...
| LE-15 | 文案分句与旁白合成 | 一句话经百炼文本模型产出脚本分句（**关闭深度思考**，见设计文档 §4.5）；TTS 合成每句并取真实音频时长；支持用户上传录音替代（转写后对齐句子，复用 LE-14 的 ASR）。T1 已完成且复审无 finding；T2 首轮 Review 的三项 P2、一项 P3 已按 TDD 修复，批次回滚不再误删旧产物、父级冲突在付费前拒绝、固定异常不携带私有路径链；26 条聚焦测试达到 100% 覆盖，81 条相关回归全绿；第二轮代码复审无 finding，唯一台账指针 finding 已补正，最终复审无 finding，证据见 `docs/development/LE-15.md` | LE-06,LE-14 | 🚧 实现中 |
...
**LE-15 T3 用户录音转写与句子对齐。** 先在 `docs/development/LE-15.md` 写定录音输入
边界、LE-14 ASR 复用面、句子对齐算法、结果与失败语义，再写真实 RED；只运行 T3
相关快速门禁，LE-15 模块全量留到 T4 收口。
