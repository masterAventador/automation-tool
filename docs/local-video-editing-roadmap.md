...
**LE-10 T1 实现、专项与分层功能验收已绿，待提交级 Review。**
新协议 `local-editing.visual-render.v1` 路径无关，Control Plane 只投影真实 Project 输出规格
与 Timeline visual 轨；真实领域链得到 `720×1280@30`、1000ms、VIDEO + IMAGE + wipe，
旁白 Material 被排除且协议没有 path/argv/codec 字段。专项 34 条、138 条语句/36 分支
100%，受影响回归 252 条。下一步提交 T1 检查点并完成 finding 修复/复审，然后进入 T2
绝对边界帧量化；完整 integration 仍留到 T6 收口时决定。
