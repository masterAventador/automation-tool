...
2. 但它要整包桌面构建才能跑，而按 `docs/development/T34-desktop-e2e-remaining.md`，
   **`main` 自 `c0cc760` 起 `tsc -b` 编译不过，整层桌面 E2E 一个都构建不起来**。
   也就是说这条唯一的运行时门禁，现在事实上没有在跑。

所以补一条**静态、快速、每次常规门禁都会跑**的规则。
...
- **桌面 E2E 当前在 `main` 上构建不起来**（T34 §4）。品牌的运行时门禁依赖它，
  在 `tsc -b` 修好之前，本次补的静态门禁是唯一在跑的那一条。这不在本任务范围内，
  但演示前值得优先修；
