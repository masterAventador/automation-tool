...
> 原始记录保留不改写，但读之前必须知道：
>
> 1. 「正式包的主窗口不向辅助功能暴露」**不成立**。本节全部观测发生在
>    13:04:22–16:09:27 的**锁屏区间**内（本节四次启动在 13:26–13:39，本文件提交于 14:19）。
>    macOS 锁屏时**每一个** App 都不向辅助功能暴露 AXWindow —— Chrome、VS Code、ghostty
>    以及一个 30 行的 AppKit 对照程序，在锁屏下全部命中本节给出的"矛盾判据"。
