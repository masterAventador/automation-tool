...
| COV-03 动效作者链 | 388 | **139** | 🚧 十一个模块已收口，见 [COV-03](COV-03.md) |
| COV-04 语音/音频 | 199 | 199 | 未开始 |
| COV-05 发布链 | 178 | 178 | 未开始 |
| COV-06 平台尾项与最终审计 | 129 | 129 | 未开始 |

全库缺口从 **2,023** 降到 **647**（COV-01 后为 1,664，减 COV-02 的 768 与
COV-03 已消除的 249）。
...
### 3.1 「断言通过但被测路径没执行」出现了四次

四次里三次是本轮新写的测试，一次是既有测试。**只看 pytest 是绿是红一次也发现不了**，
全靠缺口数字没降才暴露。所以：
...
1. **`agent.py` 123 点**——本批最大的一块，未开始。它是动效作者的主循环，缺口从
   159 一直分散到 2322 行，值得先按函数分组再动手（COV-02 §2.1 那套做法）；
