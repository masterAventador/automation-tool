...
1. **`agent.py` 剩 7 点**——契约加载、边界类型闸、流累积、静态门禁边界、模型答案
   形状、工具面反射都已做完。剩下的全在 `author()` 主流程深处：某一镜没有槽位预算
   时跳过、槽位溢出探针测量失败、静态门禁失败，以及围绕它们的两条循环边。要把完整
   流程连同一个返回特定答案的 `ScriptedModel` 一起驱动起来才够得着；
