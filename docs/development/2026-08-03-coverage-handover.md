...
| COV-03 动效作者链 | 388 | **26** | 🚧 十一个模块已收口，见 [COV-03](COV-03.md) |
| COV-04 语音/音频 | 199 | 199 | 未开始 |
| COV-05 发布链 | 178 | 178 | 未开始 |
| COV-06 平台尾项与最终审计 | 129 | 129 | 未开始 |

全库缺口从 **2,023** 降到 **534**（COV-01 后为 1,664，减 COV-02 的 768 与
COV-03 已消除的 362）。
...
- **`subTest` 套在 `assertRaises` 外面会吞掉断言失败**，然后 `caught` 未绑定，报出
  的是 `UnboundLocalError` 而不是真正的原因。两者要拆开；
- **`cmd | tail && next`：`&&` 看的是 `tail` 的退出码，永远为 0。** 本轮因此推出去
  过一次 mypy 是红的提交（下一条修掉了）。CLAUDE.md §8.3 记着这条，还是栽了——
  **命令后面不要拼东西**。

## 4. 下一步怎么排

建议顺序：**COV-03 收尾 → COV-04 → COV-05 → COV-06**。理由是 COV-03 已经开了头，
上下文还热；且剩下的 26 点里有 24 点集中在两个文件的少数几条路径上，收掉它们比
换一批模块从零开始便宜。
...
1. **`agent.py` 剩 10 点**——契约加载、边界类型闸、流累积、静态门禁边界都已做完。
   剩下的全在 `author()` 主流程里：修复轮改动过大、模型输出不是 JSON 对象、没有
   槽位预算时跳过、槽位溢出探针测量失败、静态门禁失败。这些要把一次完整的作者流程
   驱动起来才够得着（`ScriptedModel` 是现成的入口），成本比前面几批高；
