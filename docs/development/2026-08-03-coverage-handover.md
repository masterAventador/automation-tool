...
| COV-03 动效作者链 | 388 | **16** | 🚧 十一个模块已收口，见 [COV-03](COV-03.md) |
| COV-04 语音/音频 | 199 | 199 | 未开始 |
| COV-05 发布链 | 178 | 178 | 未开始 |
| COV-06 平台尾项与最终审计 | 129 | 129 | 未开始 |

全库缺口从 **2,023** 降到 **524**（COV-01 后为 1,664，减 COV-02 的 768 与
COV-03 已消除的 372）。
...
上下文还热；且剩下的 16 点里有 14 点集中在两个文件的少数几条路径上，收掉它们比
换一批模块从零开始便宜。
...
1. **`agent.py` 剩 8 点**——契约加载、边界类型闸、流累积、静态门禁边界、模型答案
   形状都已做完。剩下的全在 `author()` 主流程深处：修复轮改动超出「只改文案」、
   某一镜没有槽位预算时跳过、槽位溢出探针测量失败、静态门禁失败。要把完整流程
   连同一个返回特定错误答案的 `ScriptedModel` 一起驱动起来才够得着；
