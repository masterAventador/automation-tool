...
> 文档性质：后续开发唯一执行台账
> 建立日期：2026-07-18
> 当前阶段：Wave 9 本地 MVP 最终验收报告已建立；P9-09 保持待真实账号与双平台正式设备验收，下一独立工程任务进入 U9-01 客户 Demo 账号范围与威胁模型
...
8. `U9-01`（⬜ 未开始）：按用户授权不等待上述设备/账号事实，下一独立工程任务冻结客户 Demo 账号生命周期、登录/恢复、Session、设备归属、停用/吊销和审计威胁模型；
9. `H8-22/P9-03`（🔍 待验收）：Windows 普通包更新矩阵、Developer ID/notarization 与 Authenticode 在受控实机/签名环境补验，不阻塞上述工程任务；
10. `A7-16/A7-17`、`D6-16`、`B5-15`（🔍 待真实账号）：只在用户明确指定的自有/授权目标上补真实平台证据；账号不可用时跳过，不制造外部副作用；`B5-02` 的 Chrome FinderInfo 与未安装 Edge 也继续作为设备补验，不混入离线任务。
