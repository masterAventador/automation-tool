...
[embedded-browser-video-studio-roadmap.md](./embedded-browser-video-studio-roadmap.md) 已把 App 内置 Chromium、“智能素材成片”和“品牌动效成片”拆成 54 个候选任务。该文件当前是规划草案，不是第二份执行台账，不改变本文件中的任务状态，也不能据此宣称任何能力已实现。App 尚未发布，专项方案按全新安装设计，不迁移开发期浏览器 Profile 或旧版 App 数据；EB-02 必须最先验证 RPA 与动效渲染共用同一完整 Chromium，通过后后续任务只按单一浏览器发行物实施。
