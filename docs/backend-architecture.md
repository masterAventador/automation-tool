...
E4-03 将该入口锁为 PyInstaller 6.21.0 `onedir`：spec 直接执行 `executor/__main__.py`，冻结产物不依赖用户另装 Python；该任务完成时尚未加入 Python Playwright。B5-07 现已把 Playwright 1.61.0 作为正式运行依赖并由 spec 收集 Python driver，同时明确不执行浏览器安装、不把任何 Playwright 浏览器缓存塞入包。macOS PyInstaller framework 的包内 symlink 在清单生成前由构建期 `pyinstaller_support` 实体化；只允许解析到同一包根的普通文件/目录，逃逸、循环、特殊文件、根链接或预检后漂移全部固定拒绝。macOS arm64 与 Windows x86_64 本机均从冻结的生产 `browser_runtime.py` 以显式系统浏览器、私有 Profile、headed persistent context 完成真实启动/关闭；测试专用探针不属于正式入口，业务任务仍不会执行浏览器。GitHub macOS/Windows 矩阵使用同一实包验证；Hosted Windows Runner 仍受账户 Billing/Actions spending limit 限制，但实体机原生验收已补齐。目录 Manifest、签名、完整性和防降级仍由 E4-04/E4-05 承担，正式资源所有权由 B5-08 承担。

P9-01 在该既有 spec/Manifest 之上新增唯一 macOS 候选构建入口，不建立第二份 Executor 代码或包协议。构建的 PyInstaller config/build/dist 全在输出同级唯一临时目录，结束只保留通过双重审计的新 onedir，已有目标绝不覆盖；正式 spec 去掉 editable install 的 `direct_url.json`，候选审计再流式拒绝仓库/临时绝对路径、浏览器缓存、symlink、错误架构和缺失 Playwright driver/base library。候选审计还拒绝任何 `.framework` 目录（T129）：签一个 framework 要求 `Versions/Current` 是符号链接，而本载荷禁止 symlink，两者不可兼得，出现 framework 只可能是解释器选错（须用 uv 托管的 standalone CPython 而非 Homebrew 那种 framework 布局），拒绝信息直接给出修法。之后全部 Mach-O 统一 ad-hoc 重签并逐个 `codesign --strict`。临时测试种子只证明 E4-04 离线 Manifest 可签，不进入保留产物；Developer ID 与公证仍归 P9-03。

P9-02 在同一 spec/Manifest 上新增唯一 Windows 候选构建入口，继续只接受不存在的新输出目录，并在隔离 PyInstaller config/build/dist 中生成 onedir。复制前后都按 PE header 严格绑定当前 `x86_64|aarch64`，流式拒绝仓库/临时绝对路径，拒绝 symlink/reparse point、特殊文件、浏览器缓存、资源超限及缺失的 base library/Playwright driver。原生 runner 以一次性 Ed25519 seed 生成并验证正式 Manifest，空 stdin 启动冻结入口只能得到固定安全失败；随后只读加载系统 UIAutomationClient 并复用 E4-09 的 Windows Job Object 后代树清理验收。该 runner 已进入 Windows Desktop CI，但实际 PE/UIA/Job Object 事实仍须由 Windows 运行取得，macOS 的跨平台失败矩阵不替代原生结果。
...
H8-16B 将该启动事务的互斥范围收敛到 Installation：仓储先锁 Installation，同键重放优先返回原结果，再检查该 Installation 的全部非终态 Attempt；竞争 Task 固定抛出 `TaskDiscoveryInstallationBusy`。Alembic `20260721_0025` 把部分唯一索引从 `task_id` 改为 `installation_id`，因此不经过应用层的并发写入也只能有一个赢家；若旧库已经存在同 Installation 多条非终态 Attempt，迁移事务失败并保留 `0024`，不擅自删除或终结业务事实。HTTP 只返回 `423 installation_task_active` 和固定安全文案，不返回占用者 ID。
...
- Store 只提供独占 capture、按 ID resolve、完整引用校验 read、稳定顺序 list 与 Policy 驱动的自动 cleanup；每次操作复验根/叶 identity，拒绝未知目录项、symlink/reparse、硬链接、宽权限和文件竞态，POSIX 固定 `0700/0600`，Windows 复用私有 ACL 校验；
