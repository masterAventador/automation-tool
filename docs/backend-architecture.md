...
P9-01 在该既有 spec/Manifest 之上新增唯一 macOS 候选构建入口，不建立第二份 Executor 代码或包协议。构建的 PyInstaller config/build/dist 全在输出同级唯一临时目录，结束只保留通过双重审计的新 onedir，已有目标绝不覆盖；正式 spec 去掉 editable install 的 `direct_url.json`，候选审计再流式拒绝仓库/临时绝对路径、浏览器缓存、symlink、错误架构和缺失 Playwright driver/base library。symlink 实体化产生的 `Python.framework/Python` 冗余根别名只有与版本化二进制逐字节相同时才删除，之后全部 Mach-O 统一 ad-hoc 重签并逐个 `codesign --strict`。临时测试种子只证明 E4-04 离线 Manifest 可签，不进入保留产物；Developer ID 与公证仍归 P9-03。
